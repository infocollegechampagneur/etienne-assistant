...
RATE_LIMIT_ATTEMPTS = 30  # tentatives max
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Security(security)):
    """Dépendance pour vérifier l'authentification utilisateur (JWT)"""
...
async def get_current_admin(credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    # Sinon, vérifier JWT et s'assurer que c'est un admin
    try:
        payload = verify_token(token)
        # Pour le nouveau système, vérifier si l'utilisateur a un user_id (système de licences)
        if payload.get("user_id"):
            # Vérifier si l'email est dans la liste des admins
            # Note: Cette vérification devrait être faite côté frontend aussi
            return {"user_id": payload["user_id"], "email": payload.get("email"), "type": "jwt"}
        # Ancien système admin
