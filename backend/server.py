...
try:
    ...
    def generate_angle_diagram(angle_type: str) -> dict:
        """Génère un diagramme d'angles géométriques pour l'enseignement des mathématiques"""
        try:
            ...
            # Couleurs pour les angles
            color1 = '#3498db'  # Bleu
            color2 = '#e74c3c'  # Rouge
            color3 = '#2ecc71'  # Vert
            color4 = '#9b59b6'  # Violet
            
            if angle_type == 'adjacents':
                # Angles adjacents - partagent un côté commun
                ax.annotate('', xy=(2.5, 0), xytext=(0, 0),
                           arrowprops=dict(arrowstyle='-', color='black', lw=2))
                ax.annotate('', xy=(1.5, 2), xytext=(0, 0),
                           arrowprops=dict(arrowstyle='-', color='black', lw=2))
                ax.annotate('', xy=(-2, 1.5), xytext=(0, 0),
                           arrowprops=dict(arrowstyle='-', color='black', lw=2))
                
                # Arcs pour montrer les angles
                arc1 = plt.matplotlib.patches.Arc((0, 0), 1, 1, angle=0, theta1=0, theta2=53, color=color1, lw=3)
                arc2 = plt.matplotlib.patches.Arc((0, 0), 1.4, 1.4, angle=0, theta1=53, theta2=143, color=color2, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.8, 0.3, 'α', fontsize=16, color=color1, fontweight='bold')
                ax.text(0.2, 0.8, 'β', fontsize=16, color=color2, fontweight='bold')
                ax.set_title('ANGLES ADJACENTS\nDeux angles qui partagent un sommet et un côté commun', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• Les angles α et β sont adjacents\n• Ils ont le même sommet O\n• Ils partagent un côté commun', fontsize=11, ha='center', style='italic')
                ...
                ax.annotate('', xy=(0, 2.5), xytext=(0, 0),
                           arrowprops=dict(arrowstyle='-', color='black', lw=2))
                ax.annotate('', xy=(1.8, 1.2), xytext=(0, 0),
                           arrowprops=dict(arrowstyle='-', color='black', lw=2))
                
                # Angle droit
                rect = plt.Rectangle((0, 0), 0.3, 0.3, fill=False, color='black', lw=1.5)
                ax.add_patch(rect)
                
                arc1 = plt.matplotlib.patches.Arc((0, 0), 1, 1, angle=0, theta1=0, theta2=34, color=color1, lw=3)
                arc2 = plt.matplotlib.patches.Arc((0, 0), 1.4, 1.4, angle=0, theta1=34, theta2=90, color=color2, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.9, 0.2, 'α=34°', fontsize=12, color=color1, fontweight='bold')
                ax.text(0.3, 0.9, 'β=56°', fontsize=12, color=color2, fontweight='bold')
                ax.set_title('ANGLES COMPLÉMENTAIRES\nDeux angles dont la somme égale 90°', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• α + β = 90°\n• Exemple: 34° + 56° = 90°\n• Forment ensemble un angle droit', fontsize=11, ha='center', style='italic')
                ...
                arc1 = plt.matplotlib.patches.Arc((0, 0), 1, 1, angle=0, theta1=0, theta2=53, color=color1, lw=3)
                arc2 = plt.matplotlib.patches.Arc((0, 0), 1.3, 1.3, angle=0, theta1=53, theta2=180, color=color2, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.8, 0.3, 'α=53°', fontsize=12, color=color1, fontweight='bold')
                ax.text(-0.8, 0.6, 'β=127°', fontsize=12, color=color2, fontweight='bold')
                ax.set_title('ANGLES SUPPLÉMENTAIRES\nDeux angles dont la somme égale 180°', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• α + β = 180°\n• Exemple: 53° + 127° = 180°\n• Forment ensemble un angle plat', fontsize=11, ha='center', style='italic')
                ...
                arc1 = plt.matplotlib.patches.Arc((0, 0), 0.8, 0.8, angle=0, theta1=37, theta2=153, color=color1, lw=3)
                arc2 = plt.matplotlib.patches.Arc((0, 0), 0.8, 0.8, angle=0, theta1=217, theta2=333, color=color1, lw=3)
                arc3 = plt.matplotlib.patches.Arc((0, 0), 1.1, 1.1, angle=0, theta1=-27, theta2=37, color=color2, lw=3)
                arc4 = plt.matplotlib.patches.Arc((0, 0), 1.1, 1.1, angle=0, theta1=153, theta2=217, color=color2, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                ax.add_patch(arc3)
                ax.add_patch(arc4)
                
                ax.text(0.1, 0.6, 'α', fontsize=14, color=color1, fontweight='bold')
                ax.text(-0.3, -0.7, 'α', fontsize=14, color=color1, fontweight='bold')
                ax.text(0.6, -0.1, 'β', fontsize=14, color=color2, fontweight='bold')
                ax.text(-0.8, 0.1, 'β', fontsize=14, color=color2, fontweight='bold')
                ax.plot(0, 0, 'ko', markersize=6)
                ax.set_title('ANGLES OPPOSÉS PAR LE SOMMET\nDeux droites sécantes forment des angles égaux', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• Les angles α sont égaux entre eux\n• Les angles β sont égaux entre eux\n• α et β sont supplémentaires (α + β = 180°)', fontsize=11, ha='center', style='italic')
                
            elif angle_type == 'alternes_internes':
                # Angles alternes-internes (deux droites parallèles coupées par une sécante)
                # Droites parallèles
                ax.plot([-2.5, 2.5], [1, 1], 'b-', lw=2, label='Droite d₁')
                ax.plot([-2.5, 2.5], [-1, -1], 'b-', lw=2, label='Droite d₂')
                # Sécante
                ax.plot([-1.5, 1.5], [-2, 2], 'k-', lw=2)
                
                # Points d'intersection
                ax.plot(0.5, 1, 'ko', markersize=5)
                ax.plot(-0.5, -1, 'ko', markersize=5)
                
                # Angles alternes-internes
                arc1 = plt.matplotlib.patches.Arc((0.5, 1), 0.6, 0.6, angle=0, theta1=180, theta2=243, color=color1, lw=3)
                arc2 = plt.matplotlib.patches.Arc((-0.5, -1), 0.6, 0.6, angle=0, theta1=0, theta2=63, color=color1, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.05, 0.75, 'α', fontsize=14, color=color1, fontweight='bold')
                ax.text(-0.05, -0.75, 'α', fontsize=14, color=color1, fontweight='bold')
                ax.text(-2.3, 1.2, 'd₁', fontsize=12, color='blue')
                ax.text(-2.3, -0.8, 'd₂', fontsize=12, color='blue')
                ax.text(1.3, 1.8, 'sécante', fontsize=10, color='gray')
                
                # Symboles de parallèles
                ax.annotate('', xy=(-1.8, 1.15), xytext=(-1.8, 0.85), arrowprops=dict(arrowstyle='<->', color='green', lw=1.5))
                ax.annotate('', xy=(-1.8, -0.85), xytext=(-1.8, -1.15), arrowprops=dict(arrowstyle='<->', color='green', lw=1.5))
                
                ax.set_title('ANGLES ALTERNES-INTERNES\nDeux parallèles coupées par une sécante', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• Les angles α sont égaux (alternes-internes)\n• Situés de part et d\'autre de la sécante\n• À l\'intérieur des deux parallèles', fontsize=11, ha='center', style='italic')
                
            elif angle_type == 'alternes_externes':
                # Angles alternes-externes
                ax.plot([-2.5, 2.5], [1, 1], 'b-', lw=2)
                ax.plot([-2.5, 2.5], [-1, -1], 'b-', lw=2)
                ax.plot([-1.5, 1.5], [-2, 2], 'k-', lw=2)
                ...
                # Angles alternes-externes
                arc1 = plt.matplotlib.patches.Arc((0.5, 1), 0.6, 0.6, angle=0, theta1=0, theta2=63, color=color2, lw=3)
                arc2 = plt.matplotlib.patches.Arc((-0.5, -1), 0.6, 0.6, angle=0, theta1=180, theta2=243, color=color2, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.85, 1.25, 'β', fontsize=14, color=color2, fontweight='bold')
                ax.text(-0.95, -1.35, 'β', fontsize=14, color=color2, fontweight='bold')
                ax.text(-2.3, 1.2, 'd₁', fontsize=12, color='blue')
                ax.text(-2.3, -0.8, 'd₂', fontsize=12, color='blue')
                
                ax.set_title('ANGLES ALTERNES-EXTERNES\nDeux parallèles coupées par une sécante', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• Les angles β sont égaux (alternes-externes)\n• Situés de part et d\'autre de la sécante\n• À l\'extérieur des deux parallèles', fontsize=11, ha='center', style='italic')
                ...
                # Angles correspondants (même position relative)
                arc1 = plt.matplotlib.patches.Arc((0.5, 1), 0.6, 0.6, angle=0, theta1=0, theta2=63, color=color3, lw=3)
                arc2 = plt.matplotlib.patches.Arc((-0.5, -1), 0.6, 0.6, angle=0, theta1=0, theta2=63, color=color3, lw=3)
                ax.add_patch(arc1)
                ax.add_patch(arc2)
                
                ax.text(0.85, 1.25, 'γ', fontsize=14, color=color3, fontweight='bold')
                ax.text(-0.15, -0.75, 'γ', fontsize=14, color=color3, fontweight='bold')
                ax.text(-2.3, 1.2, 'd₁', fontsize=12, color='blue')
                ax.text(-2.3, -0.8, 'd₂', fontsize=12, color='blue')
                
                ax.set_title('ANGLES CORRESPONDANTS\nDeux parallèles coupées par une sécante', fontsize=14, fontweight='bold', pad=20)
                ax.text(0, -2.5, '• Les angles γ sont égaux (correspondants)\n• Même position relative par rapport à la sécante\n• Un au-dessus, un en-dessous des parallèles', fontsize=11, ha='center', style='italic')
            
            else:
                # Vue d'ensemble de tous les types
                ax.text(0, 0, f'Type d\'angle non reconnu: {angle_type}', fontsize=12, ha='center')
                ax.set_title('Types d\'angles disponibles', fontsize=14, fontweight='bold')
