...
async def get_site_config():
    """Récupère la config du site (public)"""
...
async def update_site_config(config: dict = Body(...), admin=Depends(get_current_admin)):
    """Met à jour la config du site (Super Admin)"""
    try:
        config.pop("_id", None)
        config["id"] = "main"
...
async def upload_site_image(file: UploadFile = File(...), admin=Depends(get_current_admin)):
    """Upload une image pour la config du site — stockée en base64 dans MongoDB"""
    try:
        import base64
