...
class GeminiQuotaManager:
    ...
    def __init__(self):
        self.max_requests_per_minute = 10  # Conservateur: éviter les 429 sur quota free (Google: 15 RPM)
        self.requests = []  # Liste des timestamps des requêtes
        self.lock = Lock()
        self.retry_after = None  # Timestamp après lequel on peut réessayer
        self.last_error_message = None
        self.consecutive_429s = 0  # Compteur de 429 consécutifs
    ...
    def get_status(self):
        ...
        with self.lock:
            ...
            # Si on est en période de retry forcé (erreur 429 de Google)
            if self.retry_after and now < self.retry_after:
                ...
                return {
                    ...
                    "can_request": False,
                    "status": "api_cooldown",
                    "message": f"API d'Étienne en pause. Réessai dans {int(wait_seconds)}s"
                }
            
            # Nettoyer le retry_after expiré et réinitialiser
            if self.retry_after and now >= self.retry_after:
                self.retry_after = None
                # Réinitialiser les requêtes pour donner une chance propre
                self.requests = []
    ...
    def record_quota_error(self, retry_delay_seconds=30):
        """Enregistrer une erreur de quota (429) avec plafond strict"""
        with self.lock:
            self.consecutive_429s += 1
            # Plafonner le délai à 30 secondes maximum, peu importe ce que Google retourne
            capped_delay = min(retry_delay_seconds, 30)
            self.retry_after = time.time() + capped_delay
            self.last_error_message = f"API cooldown: {capped_delay}s (429 #{self.consecutive_429s})"
            logging.warning(f"⚠️ Quota 429 #{self.consecutive_429s}: délai demandé={retry_delay_seconds}s, appliqué={capped_delay}s")
    
    def record_success(self):
        """Réinitialiser le compteur de 429 consécutifs après un succès"""
        with self.lock:
...
# ══════════════════════════════════════════════════════════════════════════════
# [SECTION 2b] ROTATION DE CLÉS GOOGLE API + CACHE INTELLIGENT
# ══════════════════════════════════════════════════════════════════════════════

class GeminiKeyRotator:
    """Rotation automatique entre plusieurs clés Google API"""

    DAILY_LIMIT_ESTIMATE = 20  # Quota gratuit Google : ~20 requêtes/jour/projet

    def __init__(self):
        load_dotenv(Path(__file__).parent / '.env')
        raw = os.environ.get("GOOGLE_API_KEY", "")
        # Supporte plusieurs clés séparées par des virgules
        self.keys = [k.strip() for k in raw.split(",") if k.strip()]
        self.current_index = 0
        self.failed_keys = {}  # {key: timestamp_expiry}
        self.exhausted_keys = {}  # {key: timestamp_expiry} — quota quotidien épuisé (429)
        self.usage_today = {}  # {key: nombre de requêtes aujourd'hui}
        self.usage_date = time.strftime("%Y-%m-%d")
        self.lock = Lock()
    ...
    def get_key(self):
        ...
        with self.lock:
            ...
            now = time.time()
            # Nettoyer les clés bloquées expirées (débloquer après 60s)
            self.failed_keys = {k: t for k, t in self.failed_keys.items() if t > now}
            self.exhausted_keys = {k: t for k, t in self.exhausted_keys.items() if t > now}
            # Chercher une clé disponible
            for _ in range(len(self.keys)):
                key = self.keys[self.current_index % len(self.keys)]
                self.current_index += 1
                if key not in self.failed_keys:
                    self.usage_today[key] = self.usage_today.get(key, 0) + 1
                    return key
            # Toutes les clés bloquées — retourner la première quand même
            self.usage_today[self.keys[0]] = self.usage_today.get(self.keys[0], 0) + 1
            return self.keys[0]

    def mark_failed(self, key, cooldown=60):
        """Marquer une clé comme temporairement échouée"""
        with self.lock:
            self.failed_keys[key] = time.time() + cooldown
            if cooldown >= 3600:
                # Cooldown long = quota quotidien épuisé sur ce projet
                self.exhausted_keys[key] = time.time() + cooldown
            logging.warning(f"⚠️ Clé API marquée en cooldown {cooldown}s (clé ...{key[-6:]})")

    def key_count(self):
        return len(self.keys)

    def get_keys_status(self):
        """Statut détaillé de chaque clé (masquée) pour le tableau de bord"""
        with self.lock:
            self._reset_daily_if_needed()
            now = time.time()
            self.failed_keys = {k: t for k, t in self.failed_keys.items() if t > now}
            ...
            for i, key in enumerate(self.keys):
                usage = self.usage_today.get(key, 0)
                remaining = max(0, self.DAILY_LIMIT_ESTIMATE - usage)
                if key in self.exhausted_keys:
                    status = "exhausted"
                    remaining = 0
                    cooldown_remaining = int(self.exhausted_keys[key] - now)
                elif key in self.failed_keys:
                    status = "cooldown"
                    cooldown_remaining = int(self.failed_keys[key] - now)
                else:
                    status = "active"
                ...
                keys_info.append({
                    ...
                    "usage_today": usage,
                    "daily_limit_estimate": self.DAILY_LIMIT_ESTIMATE,
                    "estimated_remaining": remaining,
                })
            return {
                "keys": keys_info,
                "total_keys": len(self.keys),
                "active_keys": sum(1 for k in keys_info if k["status"] == "active"),
                "exhausted_keys": sum(1 for k in keys_info if k["status"] == "exhausted"),
                "estimated_total_remaining": total_remaining,
                "daily_limit_per_key": self.DAILY_LIMIT_ESTIMATE,
                "usage_date": self.usage_date,
            }


key_rotator = GeminiKeyRotator()
if key_rotator.key_count() > 1:
    logging.info(f"🔑 {key_rotator.key_count()} clés Google API en rotation")
elif key_rotator.key_count() == 1:
    logging.info(f"🔑 1 clé Google API (ajoutez-en pour la rotation : GOOGLE_API_KEY=clé1,clé2,clé3)")
else:
    logging.error("❌ AUCUNE clé Google API configurée (GOOGLE_API_KEY manquant)")


class ResponseCache:
    """Cache intelligent des réponses IA dans MongoDB"""

    @staticmethod
    def _hash(message: str, message_type: str) -> str:
        """Crée un hash normalisé du message pour la comparaison"""
        normalized = message.lower().strip()
        # Retirer la ponctuation excédentaire pour matcher des questions similaires
        for ch in "!?.,;:":
            normalized = normalized.replace(ch, "")
        normalized = " ".join(normalized.split())  # Normaliser les espaces
        return hashlib.sha256(f"{message_type}:{normalized}".encode()).hexdigest()

    @staticmethod
    async def get(message: str, message_type: str):
        """Cherche une réponse en cache"""
        cache_key = ResponseCache._hash(message, message_type)
    ...
    @staticmethod
    async def store(message: str, message_type: str, response: str):
        """Stocke une réponse en cache"""
        if len(response) < 50:
            return  # Ne pas cacher les réponses trop courtes
        cache_key = ResponseCache._hash(message, message_type)
...
# Configuration de Google Gemini — la clé sera configurée dynamiquement par le rotateur
    ...
    try:
        ...
        try:
            if file_extension == 'pdf':
                ...
                # === Prétraitement automatique pour améliorer la lisibilité (manuscrits) ===
                try:
                    img = ImageOps.exif_transpose(img)
                    if img.mode != 'RGB':
                        img = img.convert('RGB')
                    # Agrandir les petites images (meilleure lecture des lettres)
                    ...
                    if max_dim < 1800:
                        ...
                        scale = 3200 / max_dim
                        img = img.resize((int(img.width * scale), int(img.height * scale)), Image.LANCZOS)
                    # Normaliser contraste/luminosité + accentuer la netteté du trait
                    img = ImageOps.autocontrast(img, cutoff=1)
                    img = ImageEnhance.Contrast(img).enhance(1.3)
                    img = ImageEnhance.Brightness(img).enhance(1.05)
                    img = ImageEnhance.Sharpness(img).enhance(1.8)
                except Exception as prep_err:
                    logging.warning(f"Prétraitement image ignoré: {prep_err}")
                ocr_prompt = """Tu es un TRANSCRIPTEUR EXPERT de copies d'élèves manuscrites. Ta mission : reproduire EXACTEMENT ce que l'élève a écrit — comme une photocopie en texte.

TON TRAVAIL SE FAIT EN 2 TEMPS POUR CHAQUE MOT :
1) DÉCHIFFRE attentivement le mot : utilise le contexte de la phrase pour IDENTIFIER les lettres difficiles (écriture cursive, lettres attachées). Prends le temps de lire la phrase complète pour bien comprendre chaque mot.
2) TRANSCRIS ensuite le mot EXACTEMENT comme l'élève l'a écrit, avec SES lettres et SES fautes. Le contexte sert à DÉCHIFFRER, jamais à CORRIGER.

⛔ INTERDICTION ABSOLUE DE CORRIGER. Cette copie sera évaluée : chaque faute a de la valeur pour l'enseignant.
   Exemples de ce que tu DOIS faire :
   - L'élève écrit « neu york » → tu écris « neu york » (PAS « New York »)
   - L'élève écrit « abendonen » → tu écris « abendonen » (PAS « abandonné »)
   - L'élève écrit « rilhouette » → tu écris « rilhouette » (PAS « silhouette »)
   - L'élève écrit « mendiai » → tu écris « mendiai » (PAS « mendiait »)

RÈGLES :
1) Transcris TOUTES les phrases, du début à la fin du texte — ne saute aucune ligne, ne laisse aucune phrase incomplète.
2) NE remplace JAMAIS un mot par sa « bonne » orthographe : recopie les lettres visibles. Si l'élève a oublié une lettre, un accent, un « s » ou un « t » final, ta transcription doit aussi l'omettre.
2b) PIÈGE FRÉQUENT — la PREMIÈRE lettre du mot : en cursive, certaines lettres se ressemblent (r/s, u/n, a/o). Examine attentivement la première lettre TRACÉE : si elle diffère de celle du mot « correct », transcris la lettre tracée (ex: si tu vois un « r » au début de ce qui ressemble à « silhouette », écris « rilhouette »). Vérifie ce point pour CHAQUE occurrence du mot, même répété.
3) IGNORE COMPLÈTEMENT les annotations de l'ENSEIGNANT en encre ROUGE : codes (G1, U1, C, P, S, V...), cercles, soulignements, flèches, notes en marge, « Titre? ». Transcris UNIQUEMENT le texte de l'élève (encre bleue/noire ou crayon).
4) Ignore les mots barrés/raturés par l'élève et le texte recouvert de correcteur blanc.
5) Respecte les retours à la ligne et paragraphes de l'élève.
6) Si un mot reste vraiment indéchiffrable après analyse du contexte, écris [illisible] — ne devine pas avec un mot bien orthographié.
7) Retourne UNIQUEMENT la transcription brute, sans commentaire.

Avant de répondre, RELIS ta transcription mot par mot en la comparant à l'image : (a) vérifie qu'aucune phrase ne manque, (b) si un mot de ta transcription est MIEUX orthographié que sur l'image, remplace-le par ce qui est réellement écrit."""
                last_ocr_error = None
                extracted_text = ""
                for _ocr_attempt in range(max(1, key_rotator.key_count())):
                    ocr_api_key = key_rotator.get_key()
                    try:
                        ...
                        if extracted_text:
                            break
                        last_ocr_error = Exception("Aucun texte détecté dans l'image")
                        break
                    except Exception as ocr_error:
                        last_ocr_error = ocr_error
                        err_str = str(ocr_error)
                        if '429' in err_str or 'quota' in err_str.lower() or 'exhausted' in err_str.lower():
                            logging.warning(f"⚠️ OCR: quota épuisé sur la clé ...{ocr_api_key[-6:] if ocr_api_key else '?'}, rotation...")
                            key_rotator.mark_failed(ocr_api_key, cooldown=3600)
                            continue
                        logging.error(f"Erreur OCR Gemini: {ocr_error}")
                        break
                if not extracted_text:
                    err_str = str(last_ocr_error or '')
                    if '429' in err_str or 'quota' in err_str.lower() or 'exhausted' in err_str.lower():
                        logging.error(f"Erreur OCR Gemini (quota): {err_str[:200]}")
                        raise HTTPException(
                            status_code=429,
                            detail="Étienne a atteint sa limite quotidienne de lecture d'images (quota gratuit épuisé). Réessayez demain, ou ajoutez des clés API de projets Google différents (GOOGLE_API_KEY=clé1,clé2,clé3)."
                        )
                    logging.error(f"Erreur OCR Gemini: {err_str[:200]}")
                    raise HTTPException(
                        status_code=400,
...
# ═══════════════════════════════════════════════════════════════════════════════
# PROFIL DE SCRIPTEUR (PS) — Taxonomies officielles par niveau (fichiers enseignante)
# ═══════════════════════════════════════════════════════════════════════════════
_PS_COHERENCE = """• C1 : Reprise de l'information (ex: Les chiens marchent. Elles avancent vite.)
• C2 : Pronom sans antécédent (on ne sait pas à qui/quoi le pronom réfère)
• C3 : Organisateurs textuels et marqueurs de relation (absence ou mauvais choix)
• C4 : Découpage des paragraphes (non ou mal découpés)
• C5 : Harmonisation des temps verbaux entre les phrases
• C6 : Autre(s)"""

_PS_VOCABULAIRE = """• V1 : Sens du mot (ex: Les enseignants apportent leurs élèves à la bibliothèque.)
• V2 : Répétition excessive d'un mot
• V3 : Variété de langue (ex: J'aime l'école pis je m'assume.)
• V4 : Anglicisme (ex: Les musiciens ont du fun.)
• V5 : Précision (ex: J'aime plein de choses.)"""

_PS_SYNTAXE_SEC1 = """• S1 : Constituants obligatoires (sujet et prédicat)
• S2 : Longueur de phrase (plusieurs phrases syntaxiques dans une même phrase graphique)
• S3 : Phrase interrogative
• S4 : Phrase négative
• S5 : Phrase calquée sur l'oral familier ou une autre langue
• S6 : Coordination (avec, et, ou)
• S7 : Complément du verbe (direct)
• S8 : Harmonisation des temps verbaux à l'intérieur de la même phrase
• S9 : Phrase à présentatif
• S10 : Autre(s)"""

_PS_SYNTAXE_SEC2PLUS = """• S1 : Constituants obligatoires (sujet et prédicat)
• S2 : Longueur de phrase (plusieurs phrases syntaxiques dans une même phrase graphique)
• S3 : Phrase interrogative
• S4 : Phrase négative
• S5 : Phrase calquée sur l'oral familier ou une autre langue
• S6 : Coordination (de phrase et de groupe)
• S7 : Choix du pronom relatif
• S8 : Choix de la préposition
• S9 : Subordonnée orpheline
• S10 : Complément du verbe (direct ou indirect)
• S11 : Harmonisation des temps verbaux à l'intérieur de la même phrase
• S12 : Phrase à présentatif
• S13 : Autre(s)"""

_PS_PONCTUATION_SEC1 = """• P1 : Virgule pour détacher un complément du nom/du pronom
• P2 : Virgule entre le sujet et le prédicat
• P3 : Virgule entre le verbe et ses expansions
• P4 : Majuscule en début de phrase
• P5 : Ponctuation de fin de phrase
• P6 : Deux-points (citation, discours rapporté, énumération)
• P7 : Positionnement du signe de ponctuation
• P8 : Autres signes de ponctuation"""

_PS_PONCTUATION_SEC2 = """• P1 : Virgule pour détacher l'organisateur textuel/marqueur de relation
• P2 : Virgule pour détacher le complément de phrase
• P3 : Virgule pour détacher un complément du nom/du pronom
• P4 : Virgule entre le sujet et le prédicat
• P5 : Virgule entre le verbe et ses expansions
• P6 : Majuscule en début de phrase
• P7 : Ponctuation de fin de phrase
• P8 : Deux-points (citation, discours rapporté, énumération)
• P9 : Positionnement du signe de ponctuation
• P10 : Autres signes de ponctuation"""

_PS_PONCTUATION_SEC3PLUS = """• P1 : Virgule pour détacher l'organisateur textuel/marqueur de relation
• P2 : Virgule de coordonnant (mais, car, puis...)
• P3 : Virgule pour détacher l'apostrophe (interpellation)
• P4 : Virgule pour détacher une phrase incise ou incidente
• P5 : Virgule pour détacher le complément de phrase
• P6 : Virgule pour détacher un complément du nom/du pronom
• P7 : Virgule pour détacher un complément de l'adjectif
• P8 : Virgule entre le sujet et le prédicat
• P9 : Virgule entre le verbe et ses expansions
• P10 : Majuscule en début de phrase
• P11 : Ponctuation de fin de phrase
• P12 : Deux-points (citation, discours rapporté, énumération)
• P13 : Positionnement du signe de ponctuation
• P14 : Autres signes de ponctuation"""

_PS_GRAMMAIRE_SEC1 = """• G1 : Classe de mot (a/à, son/sont, ces/c'est, ou/où, ce/se, on/ont)
• G2 : Accord dans le groupe nominal (GN)
• G3 : Accord du verbe conjugué (GV)
• G4 : Conjugaison (radical et terminaison)
• G5 : Choix de l'auxiliaire avoir ou être
• G6 : Accord du participe passé avec l'auxiliaire être
• G7 : Autre(s)"""

_PS_GRAMMAIRE_SEC2 = _PS_GRAMMAIRE_SEC1.replace("• G7 : Autre(s)", """• G7 : Pronom de reprise (même phrase)
• G8 : Autre(s)""")

_PS_GRAMMAIRE_SEC3PLUS = _PS_GRAMMAIRE_SEC1.replace("• G7 : Autre(s)", """• G7 : Accord du participe passé avec l'auxiliaire avoir
• G8 : Terminaison ER/É
• G9 : Pronom de reprise (même phrase)
• G10 : Autre(s)""")

PS_TAXONOMIES = {
    "sec1": {"syntaxe": _PS_SYNTAXE_SEC1, "ponctuation": _PS_PONCTUATION_SEC1, "grammaire": _PS_GRAMMAIRE_SEC1},
    "sec2": {"syntaxe": _PS_SYNTAXE_SEC2PLUS, "ponctuation": _PS_PONCTUATION_SEC2, "grammaire": _PS_GRAMMAIRE_SEC2},
    "sec3": {"syntaxe": _PS_SYNTAXE_SEC2PLUS, "ponctuation": _PS_PONCTUATION_SEC3PLUS, "grammaire": _PS_GRAMMAIRE_SEC3PLUS},
    "sec4": {"syntaxe": _PS_SYNTAXE_SEC2PLUS, "ponctuation": _PS_PONCTUATION_SEC3PLUS, "grammaire": _PS_GRAMMAIRE_SEC3PLUS},
    "sec5": {"syntaxe": _PS_SYNTAXE_SEC2PLUS, "ponctuation": _PS_PONCTUATION_SEC3PLUS, "grammaire": _PS_GRAMMAIRE_SEC3PLUS},
}

def build_ps_instructions(niveau: str) -> str:
    """Construit les instructions du Profil de Scripteur (PS) pour le niveau donné"""
    tax = PS_TAXONOMIES.get(niveau, PS_TAXONOMIES["sec3"])
    niveau_label = niveau.replace("sec", "Secondaire ")
    return f"""

════════════════════════════════════════
PARTIE D — PROFIL DE SCRIPTEUR (PS) — {niveau_label} (OBLIGATOIRE)
════════════════════════════════════════
Après le tableau de correction et la note finale, produis le PROFIL DE SCRIPTEUR de l'élève.
C'est un tableau diagnostique qui classe CHAQUE erreur du texte selon la taxonomie officielle ci-dessous.
⚠️ ATTENTION : dans le PS, les codes C1-C6 désignent des types d'erreurs de COHÉRENCE
(à ne pas confondre avec les critères d'évaluation C1-C5 du MEQ).

TAXONOMIE OFFICIELLE {niveau_label} :

**Critère 2 — Cohérence du texte (C) :**
{_PS_COHERENCE}

**Critère 3 — Vocabulaire (V) :**
{_PS_VOCABULAIRE}

**Critère 4 — Syntaxe (S) :**
{tax['syntaxe']}

**Critère 4 — Ponctuation (P) :**
{tax['ponctuation']}

**Critère 5 — Orthographe d'usage (U) :**
• U1 : Orthographe d'usage (ex: menger, julie, Noel, inteligent)

**Critère 5 — Grammaire / accords (G) :**
{tax['grammaire']}

FORMAT DU PROFIL DE SCRIPTEUR (tableau markdown) :

## 📋 Mon profil de scripteur.rice — {niveau_label}

| Code | Type d'erreur | Nombre | Exemples tirés du texte |
|------|---------------|--------|-------------------------|
(Une ligne par code de la taxonomie AYANT au moins 1 erreur. Regroupe visuellement par section : **Cohérence (C)**, **Vocabulaire (V)**, **Syntaxe (S)**, **Ponctuation (P)**, **Orthographe d'usage (U)**, **Grammaire (G)**.)

Puis termine par le BILAN :
| Bilan | Valeur |
|-------|--------|
| Total des erreurs S+P (Critère 4) | X |
| Total des erreurs U+G (Critère 5) | X |
| Nombre de mots total du texte | X |
| Pourcentage d'erreur Critère 4 (S+P ÷ mots × 100) | X % |
| Pourcentage d'erreur Critère 5 (U+G ÷ mots × 100) | X % |

Termine le PS par 2-3 recommandations personnalisées : les 2 ou 3 types d'erreurs les plus
fréquents chez cet élève et une piste de travail concrète pour chacun (avec outils : Antidote, Usito, WordQ).

⚠️ OBLIGATOIRE — À LA TOUTE FIN de ta réponse (dernière ligne, après tout le reste), ajoute ce bloc
de données machine EXACTEMENT dans ce format (JSON valide sur UNE SEULE ligne, sans explication autour) :
[PS_DATA]{{"niveau":"{niveau}","mots":<nombre de mots du texte>,"total_sp":<total erreurs S+P>,"total_ug":<total erreurs U+G>,"erreurs":{{"<code>":<nombre>}}}}[/PS_DATA]
Dans "erreurs", inclus UNIQUEMENT les codes ayant au moins 1 erreur (codes possibles : C1-C6, V1-V5, S1-S13, P1-P14, U1, G1-G10).
"""

# ═══════════════════════════════════════════════════════════════════════════════
# PROGRESSION DES APPRENTISSAGES (MELS 2011) — Repères par niveau pour C1 à C5
# ═══════════════════════════════════════════════════════════════════════════════
PDA_PAR_NIVEAU = {
    "1": """**Secondaire 1 — Ce qui est enseigné/attendu :**
- C1 (adaptation) : description, récit en 5 étapes (schéma narratif), poésie; justification amorcée
- C2 (cohérence) : reprise de l'information par pronom et GN simple; découpage en paragraphes; harmonisation de base des temps du récit
- C3 (vocabulaire) : vocabulaire concret et précis, champs lexicaux, registre standard
- C4 (syntaxe/ponct.) : phrase de base et constituants obligatoires; types et formes de phrases (interrogative, négative); ponctuation de fin de phrase; virgule dans l'énumération
- C5 (orthographe) : accords dans le GN (dét+nom+adj); accord sujet-verbe (sujet proche du verbe); temps simples de l'indicatif; homophones fréquents (a/à, son/sont, on/ont, ou/où)
- EN COURS D'ACQUISITION (indulgence) : participe passé avec être, virgule du complément de phrase déplacé, subordonnées
- NON ENCORE ENSEIGNÉ (signaler SANS pénaliser sévèrement) : PP avec avoir, concordance des temps avancée, subordonnées complexes""",
    "2": """**Secondaire 2 — Ce qui est enseigné/attendu :**
- C1 : description, justification, narration élaborée, poésie
- C2 : organisateurs textuels variés; reprises variées (pronoms, synonymes, GN); harmonisation des temps du récit
- C3 : vocabulaire varié et adapté, synonymie pour éviter les répétitions
- C4 : subordonnée relative (qui, que); virgule pour l'apostrophe et le complément de phrase déplacé; discours direct (deux-points, guillemets)
- C5 : MAÎTRISE exigée des notions de sec 1 + accord sujet-verbe avec écran/inversion; PP avec être; homophones (ces/ses, c'est/s'est, leur/leurs)
- EN COURS D'ACQUISITION : subordonnées circonstancielles, conditionnel et subjonctif présent
- NON ENCORE ENSEIGNÉ : PP avec avoir (CD avant), PP des verbes pronominaux, concordance avancée""",
    "3": """**Secondaire 3 — Ce qui est enseigné/attendu :**
- C1 : explication (pourquoi/comment avec procédés explicatifs), narration complexe (retours en arrière), justification appuyée, poésie
- C2 : progression continuité/rupture, non-contradiction; concordance des temps de base
- C3 : vocabulaire précis et nuancé, termes techniques expliqués
- C4 : subordonnées relatives (dont, où, préposition+relative), complétives, circonstancielles (temps, but, cause); virgule de coordonnant et d'incise; discours indirect amorcé
- C5 : MAÎTRISE exigée des notions de sec 1-2 + PP avec avoir (CD placé avant); terminaisons é/er/ez; subjonctif
- EN COURS D'ACQUISITION : réduction de subordonnées, modalisation
- NON ENCORE ENSEIGNÉ : PP des verbes pronominaux, PP suivi d'un infinitif""",
    "4": """**Secondaire 4 — Ce qui est enseigné/attendu :**
- C1 : argumentation (thèse, arguments appuyés, contre-thèse amorcée), narration psychologique, poésie; modalisation
- C2 : point de vue maintenu; reprises complexes (nominalisation); concordance des temps étendue
- C3 : vocabulaire nuancé, connotation, figures de style
- C4 : subordonnées de concession/opposition/hypothèse; réduction de subordonnées; ponctuation expressive (point-virgule, deux-points explicatif); discours rapporté indirect
- C5 : MAÎTRISE exigée des notions de sec 1-3 + PP des verbes pronominaux (amorce); accords avec sujets collectifs; participe présent vs adjectif verbal
- EN COURS D'ACQUISITION : PP pronominaux (cas complexes), PP suivi d'un infinitif""",
    "5": """**Secondaire 5 — Ce qui est enseigné/attendu (MAÎTRISE COMPLÈTE) :**
- C1 : argumentation complète (stratégie, réfutation, concession); adaptation fine au destinataire et au genre
- C2 : cohérence maîtrisée; transitions élaborées; concordance des temps maîtrisée
- C3 : vocabulaire riche, précis, registre parfaitement adapté
- C4 : maîtrise de TOUTE la syntaxe (juxtaposition, coordination, subordination) et de TOUTE la ponctuation
- C5 : maîtrise EXIGÉE de TOUS les accords, y compris PP des verbes pronominaux et PP suivi d'un infinitif — AUCUNE indulgence sur les notions des niveaux antérieurs""",
}

def build_pda_instructions(niveau_num: str) -> str:
    """Construit le bloc Progression des apprentissages (MELS 2011) pour la correction"""
    pda = PDA_PAR_NIVEAU.get(niveau_num)
    if not pda:
        return ""
    return f"""

════════════════════════════════════════
PROGRESSION DES APPRENTISSAGES (MELS 2011) — CALIBRAGE OBLIGATOIRE DES CRITÈRES C1 À C5
════════════════════════════════════════
Ajuste ta sévérité de correction selon la Progression des apprentissages officielle du MELS :

{pda}

RÈGLES DE CALIBRAGE :
1. Notion des niveaux ANTÉRIEURS ou marquée MAÎTRISE : erreur comptée PLEINEMENT.
2. Notion EN COURS D'ACQUISITION au niveau de l'élève : erreur comptée, mais accompagne-la d'un commentaire formatif encourageant.
3. Notion NON ENCORE ENSEIGNÉE à ce niveau : SIGNALE l'erreur dans le texte annoté (pour information), mais indique « (notion non enseignée — non comptée) » dans le tableau et NE LA COMPTE PAS dans le total C4/C5.
4. Dans ton évaluation de C1, C2 et C3, tiens compte de ce qui est réellement attendu à ce niveau selon la progression ci-dessus (n'exige pas d'un élève de sec 1 la maîtrise argumentative d'un sec 5).
"""


async def check_plagiarism_web(student_text: str) -> str:
    """Recherche sur le web des passages potentiellement plagiés. Retourne un résumé ou None."""
    try:
        from google import genai as genai_new_sdk
        from google.genai import types as genai_new_types

        api_key = key_rotator.get_key()
        client = genai_new_sdk.Client(api_key=api_key)
        config = genai_new_types.GenerateContentConfig(
            tools=[genai_new_types.Tool(google_search=genai_new_types.GoogleSearch())]
        )
        prompt = f"""Tu es un détecteur de plagiat scolaire. Voici un texte d'élève du secondaire québécois.

1. Identifie les 3 à 5 phrases les PLUS distinctives/élaborées du texte (formulations peu communes).
2. Recherche sur le web si ces phrases (ou des passages très similaires) existent déjà en ligne.
3. Pour chaque correspondance trouvée, rapporte : la phrase exacte du texte de l'élève, la source web (titre + URL), et le degré de similarité (identique / très similaire / paraphrase probable).
4. Si AUCUNE correspondance n'est trouvée, dis-le clairement.

Réponds de façon factuelle et concise en français.
...
{student_text[:6000]}"""
            ...
            for cand in (response.candidates or []):
                ...
                if gm and getattr(gm, 'grounding_chunks', None):
                    for chunk in gm.grounding_chunks:
                        ...
                        if web and web.uri:
                            sources.append(f"- {web.title or 'Source web'}: {web.uri}")
            if sources:
        ...
        logging.warning(f"⚠️ Recherche web plagiat indisponible ({str(e)[:120]}), analyse stylistique seule.")
        return None

def build_plagiat_instructions(web_findings: str = None) -> str:
    """Construit les instructions de détection de plagiat pour la correction"""
    web_section = ""
    if web_findings:
        web_section = f"""
RÉSULTATS DE LA RECHERCHE WEB EFFECTUÉE (à intégrer dans ton rapport) :
{web_findings}
"""
    else:
        web_section = """
(La recherche web n'a pas retourné de résultat exploitable — appuie-toi sur l'analyse stylistique
et indique honnêtement qu'aucune source web n'a pu être confirmée.)
"""
    return f"""

════════════════════════════════════════
PARTIE E — DÉTECTION DE PLAGIAT (OBLIGATOIRE)
════════════════════════════════════════
Produis un rapport de détection de plagiat en combinant DEUX approches :

**1. Analyse stylistique** : repère les passages dont le style tranche avec le reste du texte
(vocabulaire soudainement soutenu, syntaxe complexe inhabituelle pour le niveau, absence
d'erreurs dans un passage alors que le reste en contient, rupture de ton, tournures d'adulte).

**2. Correspondances web** :{web_section}

FORMAT DU RAPPORT :

## 🔍 Rapport de détection de plagiat

**Indice global de plagiat : X %** (estimation honnête : faible 0-20%, modéré 21-50%, élevé 51-100%)

Dans le TEXTE ANNOTÉ (Partie A), souligne les passages suspects de plagiat avec cette balise GRISE :
<span style="background-color:#E5E7EB;border-bottom:2px dashed #6B7280;padding:1px 3px;border-radius:3px">passage suspect</span>

Puis présente le tableau :
| Passage suspect | Raison du soupçon | Source probable (URL si trouvée) | Similarité |
|-----------------|-------------------|----------------------------------|------------|

Si le texte semble authentique, dis-le clairement : « ✅ Aucun indice de plagiat détecté — le texte
semble authentique et cohérent avec le niveau de l'élève. »
⚠️ Rappelle à l'enseignant que la détection de plagiat par IA est un INDICE, pas une preuve.
"""
...
Si une erreur d'USAGE (U) est RÉPÉTÉE (même mot, même erreur d'usage), utilise un style barré pour les occurrences suivantes:
<span style="background-color:#BFDBFE;padding:1px 3px;border-radius:3px;text-decoration:line-through">[N*]mot</span>
(le * indique "déjà compté en usage, voir erreur #X")
⚠️ Les erreurs GRAMMATICALES (G) répétées ne sont PAS barrées — chaque occurrence compte comme erreur distincte.
...
════════════════════════════════════════
PARTIE B — TABLEAU DE CORRECTION
════════════════════════════════════════
Après le texte annoté, présente le tableau de correction.
Le FORMAT DU TABLEAU est dicté par le choix de l'enseignant dans le formulaire:

**OPTION 1 — Par numéro (ordre d'apparition dans le texte):**
...
| 3* | "journer" (2e occ.) | C5 | U | Bleu | Même erreur U que #3 — non comptée | "journée" |

**OPTION 2 — Par type de faute (regroupé par catégorie):**
| Type | # | Erreur | Critère | Explication | Correction |
|------|---|--------|---------|-------------|------------|
| **G - Orth. grammaticale** | | | | | |
| | 1 | "les chien" | C5 | Accord pluriel manquant | "les chiens" |
| | 2 | "a passer" | C5 | Participe passé | "a passé" |
| **U - Orth. usage** | | | | | |
| | 3 | "journer" | C5 | Orthographe d'usage | "journée" |
| | 3* | "journer" (2e occ.) | C5 | Même erreur U #3 — non comptée | "journée" |
| **S - Syntaxe** | | | | | |
| ...  | | | | | |

⚠️ IMPORTANT: Utilise l'option demandée par l'enseignant. Si non précisé, utiliser l'option 1 (par numéro).
Si l'enseignant demande "Option 3 — Les deux", présente D'ABORD le tableau par numéro (option 1), puis
immédiatement après, le MÊME tableau réorganisé par type de faute (option 2).
...
1. COMPTER le nombre total d'erreurs pour C4 (syntaxe + ponctuation) et C5 (orthographe usage + grammaticale)
   ...
   - Orthographe d'usage (U): même mot, même erreur = compter 1 SEULE FOIS
   - Orthographe grammaticale (G): compter CHAQUE occurrence (sauf éléments liés au même donneur = 1G)
...
- Phrase SYNTAXIQUE = phrase de base OU phrase transformée OU phrase à construction particulière
- Phrase GRAPHIQUE = commence par une majuscule et termine par un point (peut contenir plusieurs phrases syntaxiques)

RÈGLES DE COMPTAGE C4:
- Ne compter qu'UNE erreur de syntaxe par phrase SYNTAXIQUE (pas graphique!)
  Ex: "*Les bateaux () peuvent pas se rendre (dans) ce continent" = 1S (même phrase syntaxique)
  Ex: "*L'étude d'une carotte de glace qui provient (dans) l'Antarctique aide les chercheurs
       (pour) mieux comprendre () histoire du climat." = 2S (2 phrases syntaxiques différentes:
       la relative "qui provient..." = 1S et la principale "...aide...(pour)...() histoire" = 1S)
- Quand une erreur de syntaxe contient aussi une erreur d'orthographe,
  la SYNTAXE a priorité (compter 1S, pas 1S+1U ou 1S+1G)
- Compter UNE seule erreur pour les signes de ponctuation utilisés par paire
  ...
  Ex: "*Roald Amundensen () un explorateur norvégien () a été le premier..." = 1P (virgules paire)
- Compter une erreur de ponctuation pour chaque faute DISTINCTE commise
  Ex: "*En Antarctique() il existe des animaux... protégés ()" = 2P (2 ponctuation distinctes)
...
- Erreur d'emploi d'un pronom selon sa FONCTION = critère 4
  Ex: "L'équipe d'aventuriers (lui) a invité" → C4 (pronom mauvaise fonction)
- Majuscule absente en début de phrase graphique ou inappropriée après ponctuation = critère 4
- Autres erreurs de majuscule (noms propres, etc.) = critère 5 (orthographe d'usage)

📌 RÈGLES CRITÈRE 5 (Orthographe d'usage et grammaticale):
- Ne compter qu'UNE erreur par mot (ex: "*explorratteurs" = 1U, pas 2)
- L'orthographe GRAMMATICALE a priorité sur l'orthographe d'usage
  Ex: "*les lacs sous-glacière" = 1G (pas 1U+1G, le G absorbe le U)
...
- Nombres de 1 à 9 écrits en chiffre = 1 seule erreur U par texte entier
...
- Graphie rectifiée ET traditionnelle sont toutes deux acceptées
  (l'élève peut mélanger les deux dans le même texte)
- Les erreurs de formation du genre du nom et de l'adjectif = orthographe d'USAGE
  Ex: "*espione" au lieu de "espionne" = 1U (pas 1G)
- Considérer UNIQUEMENT les verbes et les temps à l'étude selon la progression
  des apprentissages du niveau de l'élève

⚠️ RÈGLES DE COMPTAGE DIFFÉRENCIÉES SELON LE TYPE DE FAUTE:

📌 ORTHOGRAPHE D'USAGE (U) — Fautes répétées:
- Si la MÊME erreur d'usage se répète pour le MÊME MOT → compter 1 SEULE FOIS
- MAIS si l'erreur d'usage est DIFFÉRENTE pour le même mot → compter CHAQUE FOIS
- Toujours SOULIGNER et IDENTIFIER chaque occurrence dans le texte (barré pour les non-comptées)
- Indiquer dans le tableau: "Même erreur que #X — non comptée" pour les occurrences U répétées
Exemple: "*journer" 3 fois → 1U compté, mais les 3 occurrences sont annotées (2 barrées)

📌 ORTHOGRAPHE GRAMMATICALE (G) — Fautes répétées:
- ⚠️ COMPTER CHAQUE FOIS qu'une erreur grammaticale est répétée, même si c'est le même mot
- Chaque occurrence est une erreur distincte car l'élève a échoué à appliquer la règle à chaque fois
- NE PAS barrer les occurrences suivantes — chacune compte comme une erreur séparée
Exemple: "*les chien" 3 fois → 3G comptées (3 erreurs distinctes d'accord)

📌 EXCEPTIONS POUR G — Éléments liés au même donneur:
- Si plusieurs mots (déterminant, adjectif, nom, participe passé) sont régis par la MÊME RÈGLE
  et liés au MÊME donneur (nom/pronom) → compter 1 SEULE erreur G pour le groupe entier
- CECI EST OBLIGATOIRE. Ne jamais compter 2G pour un déterminant + son nom non accordés.
Exemples OBLIGATOIRES à respecter:
  "*quelque manège" → 1G (déterminant "quelque" + nom "manège" = même donneur, même règle d'accord pluriel)
  "*L'Antarctique est (une) immense espace (recouverte) de glace" = 1G (même donneur "espace")
  "*Des carottes de glace sont (recueillie) et (analysée) par des chercheurs" = 1G (même donneur "carottes")
  "*cette villes" = 1G (déterminant + nom, même donneur)
  "*de grand parc d'attraction" = 1G (adjectif + nom + complément, même règle pluriel)
⚠️ ATTENTION: Quand un déterminant ET le nom qui le suit sont TOUS DEUX mal accordés
   pour la MÊME raison (genre ou nombre), c'est TOUJOURS 1 seule erreur G, JAMAIS 2.

📌 EXCEPTIONS POUR G — Verbes même personne:
- Accord incorrect du MÊME sujet avec plusieurs verbes → 1 seule erreur G
Exemple: "*Je vous présenterez et décrirez l'Antarctique" = 1G (accord avec "vous" pour les 2 verbes)

📌 SYNTAXE (S) — Fautes répétées:
- Compter une erreur S par occurrence, mais MAXIMUM 1 erreur S par phrase syntaxique
- Si la même structure fautive apparaît dans des phrases DIFFÉRENTES → compter dans chaque phrase
Exemple: "*Les bateaux () peuvent pas" dans phrase 1 + "*Il () veut pas" dans phrase 2 → 2S

📌 PONCTUATION (P):
- Chaque erreur de ponctuation distincte est comptée séparément
- Ponctuation par paire (guillemets, tirets, parenthèses) mal utilisée = 1P pour la paire

📌 VOCABULAIRE (V):
- Répétition de mots NON liée à la reprise d'information → évaluer sous critère 3
- Imprécision, incorrection ou manque de variété → chaque occurrence est évaluée
...
ORGANISATION: Marques d'organisation (titre, intertitres, paragraphes, organisateurs textuels).
  - Le découpage en paragraphes est important MAIS ne doit PAS être un facteur discriminant d'emblée.
  - Ne PAS pénaliser l'absence d'organisateurs textuels en début de paragraphe si cela n'entraîne pas de rupture.
  - Le développement peut être en un seul paragraphe si le découpage n'est pas nécessaire.
CONTINUITÉ: Reprise de l'information d'une phrase à l'autre (répétition, substitution, nominalisation).
  - L'élève doit éviter les répétitions inutiles.
  - Les exigences à l'égard de la substitution doivent être raisonnables.
  - Erreur de genre/nombre d'un pronom dont l'antécédent est dans une AUTRE phrase graphique = C2
    Ex: "La banquise augmente. *Il empêche les bateaux" = C2 (antécédent dans phrase précédente)
PROGRESSION: Introduction de nouveaux éléments, marqueurs de relation, harmonisation des temps entre phrases.
  - L'harmonisation des temps verbaux ENTRE les phrases = critère 2 (pas critère 4).

CRITÈRE 3 - Vocabulaire approprié:
Indicateurs: Variété de langue, norme (impropriétés, barbarismes, anglicismes)
Sous-critères détaillés:
PRÉCISION: Mots qui désignent avec netteté et sans ambiguïté. Éviter les mots vagues.
  - Les verbes avoir, être et faire sont souvent considérés imprécis, MAIS leur emploi est
    tout à fait correct dans certains contextes. Ne PAS les pénaliser systématiquement.
JUSTESSE: Mots appropriés qui respectent le sens du dictionnaire usuel et appartiennent à la langue
  standard. Éviter les impropriétés, barbarismes, anglicismes.
RICHESSE: Mots recherchés et variés. L'élève utilise différentes expressions pour évoquer la même idée.
  - Note: La répétition de mots NON liée à la reprise de l'information est évaluée au critère 3 (pas C2).
...
Précisions:
  - Phrase syntaxique = phrase de base, transformée ou à construction particulière
  - Phrase graphique = majuscule → point (peut contenir plusieurs phrases syntaxiques)
  - Erreur d'emploi d'un pronom selon sa fonction = C4 (ex: "*lui a invité" au lieu de "*l'a invité")
  - Erreur de genre/nombre d'un pronom dont l'antécédent est dans la MÊME phrase = C5
  - Majuscule absente en début de phrase graphique = C4; autres majuscules (noms propres) = C5
...
Précisions:
  - Graphie traditionnelle ET rectifiée sont TOUTES DEUX acceptées (mélange OK dans le même texte)
  - Erreurs de formation du genre du nom/adjectif = orthographe d'USAGE (ex: *espione = 1U)
  - Ne considérer QUE les verbes et temps à l'étude selon la progression des apprentissages du niveau
  - L'orthographe grammaticale a PRIORITÉ sur l'orthographe d'usage (ex: *sous-glacière = 1G, pas 1U+1G)
...
📋 GRILLES D'ÉVALUATION — RÈGLE OBLIGATOIRE:
Quand tu crées un plan de cours, une activité, une SAÉ, un travail ou un examen,
TOUJOURS proposer une GRILLE D'ÉVALUATION sous forme de tableau avec:
- Les critères d'évaluation en lignes (adaptés au PFEQ)
- Les niveaux de performance en colonnes (A/B/C/D/E ou Excellent/Très bien/Bien/Acceptable/Insuffisant)
- Un espace pour la note (/X points ou lettre ou %)
- Les descripteurs pour chaque case

Format de la grille:
| Critère | Excellent (A) | Très bien (B) | Bien (C) | Acceptable (D) | Insuffisant (E) | Note |
|---------|--------------|---------------|----------|-----------------|-----------------|------|
| [Critère 1] | [description A] | [description B] | [description C] | [description D] | [description E] | /X |
| [Critère 2] | ... | ... | ... | ... | ... | /X |
| **Total** | | | | | | **/XX** |

Si le plan de cours ne comporte pas d'évaluation directe, propose quand même une grille d'observation ou d'appréciation.
...
📋 GRILLES D'ÉVALUATION — TOUJOURS inclure une grille d'évaluation sous forme de tableau:
| Critère | Excellent (A) | Très bien (B) | Bien (C) | Acceptable (D) | Insuffisant (E) | Note |
|---------|--------------|---------------|----------|-----------------|-----------------|------|
Avec descripteurs clairs par niveau et espace pour la note (/X, lettre, ou %).
...
📋 GRILLES D'ÉVALUATION — TOUJOURS inclure une grille d'évaluation ou d'observation sous forme de tableau:
| Critère | Excellent (A) | Très bien (B) | Bien (C) | Acceptable (D) | Insuffisant (E) | Note |
|---------|--------------|---------------|----------|-----------------|-----------------|------|
Avec descripteurs clairs par niveau et espace pour la note (/X, lettre, ou %).
...
AT THE END, add a "📚 Sources and References" section.{branding_instruction_en}"""
        ...
        # === CORRECTION AVANCÉE: Profil scripteur (PS) + Détection de plagiat ===
        ps_match = re.search(r'\[PROFIL_SCRIPTEUR:(sec[1-5])\]', enhanced_message)
        if ps_match:
            enhanced_message = enhanced_message.replace(ps_match.group(0), '').strip()
            enhanced_message += build_ps_instructions(ps_match.group(1))

        # === PROGRESSION DES APPRENTISSAGES (MELS 2011) — auto-injectée lors des corrections ===
        if "TEXTE DE L'ÉLÈVE À CORRIGER" in enhanced_message:
            niveau_match = re.search(r'Niveau:\*{0,2}\s*Secondaire\s*([1-5])', enhanced_message)
            if niveau_match:
                enhanced_message += build_pda_instructions(niveau_match.group(1))

        if '[DETECTION_PLAGIAT]' in enhanced_message:
            enhanced_message = enhanced_message.replace('[DETECTION_PLAGIAT]', '').strip()
            student_text = ''
            if "TEXTE DE L'ÉLÈVE À CORRIGER:" in enhanced_message:
                student_text = enhanced_message.split("TEXTE DE L'ÉLÈVE À CORRIGER:")[-1].replace('**', '').strip()
            web_findings = await check_plagiarism_web(student_text) if len(student_text) > 50 else None
            enhanced_message += build_plagiat_instructions(web_findings)
        ...
        # === CACHE INTELLIGENT — Vérifier si une réponse similaire existe ===
        cached_response = await ResponseCache.get(message, message_type)
        if cached_response:
            ...
            return {
                ...
                "can_download": len(cached_response) > 50
            }

        # === GOOGLE GEMINI 2.5 FLASH — ROTATION DE CLÉS + RETRY PATIENT ===
        last_error = None
        max_attempts = 4  # 4 tentatives avec pauses croissantes
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Rotation de clé à chaque tentative
                api_key = key_rotator.get_key()
                ...
                if chat_history:
                    ...
                    response = await asyncio.to_thread(model.generate_content, enhanced_message)

                response_text = response.text
                gemini_quota.record_success()

                # Stocker en cache pour les futures requêtes identiques
                await ResponseCache.store(message, message_type, response_text)
                ...
                is_429 = "429" in error_str or "resource has been exhausted" in error_lower or "quota" in error_lower

                if is_429:
                    # Marquer cette clé en échec et basculer
                    key_rotator.mark_failed(api_key, cooldown=65)
                    logging.info(f"🔄 429 sur clé ...{api_key[-6:]}, tentative {attempt + 1}/{max_attempts}")

                    if attempt < max_attempts - 1:
                        # Pauses croissantes : 3s, 8s, 20s — laisse le rate limit se réinitialiser
                        waits = [3, 8, 20]
                        wait = waits[min(attempt, len(waits) - 1)]
                        logging.info(f"   Attente {wait}s puis rotation vers clé suivante...")
                        await asyncio.sleep(wait)
                        continue

                    # Toutes les tentatives échouées — NE PAS bloquer longtemps
                    logging.warning(f"⚠️ 429 après {max_attempts} tentatives: {error_str[:150]}")
                    gemini_quota.record_quota_error(10)
                    return {
                        "response": "⏳ Étienne a reçu beaucoup de demandes. Veuillez patienter 10 secondes et réessayer. Astuce : les questions déjà posées sont instantanées grâce au cache.",
                        "trust_score": None,
                        "sources": [],
                        "quota_exceeded": True,
                        "wait_seconds": 10
...
async def get_quota_status():
    ...
    status = gemini_quota.get_status()
    status["api_keys_count"] = key_rotator.key_count()
    status["api_keys_failed"] = len(key_rotator.failed_keys)
    return status

@api_router.get("/keys-status")
async def get_keys_status():
    """Tableau de bord des clés API Google : statut, quota estimé restant, cooldowns"""
    return key_rotator.get_keys_status()
...
async def chat_with_ai(request: ChatRequest, http_request: Request):
    ...
    try:
        ...
        # === PROFIL SCRIPTEUR: extraire et retirer le bloc de données machine [PS_DATA] ===
        ps_profile_data = None
        ps_student_name = None
        ps_data_match = re.search(r'\[PS_DATA\]\s*(\{.*?\})\s*\[/PS_DATA\]', ai_result["response"], re.DOTALL)
        if ps_data_match:
            try:
                ps_profile_data = json.loads(ps_data_match.group(1))
            except Exception:
                ps_profile_data = None
            ai_result["response"] = ai_result["response"].replace(ps_data_match.group(0), '').rstrip()
            eleve_match = re.search(r'\*\*Élève:\*\*\s*([^\n]+)', request.message)
            if eleve_match:
                ps_student_name = eleve_match.group(1).strip()
        ps_groupe_match = re.search(r'\*\*Groupe:\*\*\s*([^\n]+)', request.message)
        ps_groupe = ps_groupe_match.group(1).strip() if ps_groupe_match else ""
        ...
        # Persistance serveur: sauvegarder la réponse dans la conversation liée
        # (évite la perte d'une longue correction si l'utilisateur quitte la page)
        if request.conversation_id:
            try:
                ...
                if ai_result.get("image_base64"):
                    assistant_msg["image_base64"] = ai_result["image_base64"]
                ...
                logging.warning(f"Persistance conversation échouée: {conv_err}")
        ...
        # === PROFIL SCRIPTEUR: sauvegarder l'entrée de suivi de l'élève ===
        if ps_profile_data and ps_student_name and user_email:
            try:
                counts = ps_profile_data.get("erreurs", {}) or {}
                fingerprint = f"{ps_student_name}|{ps_profile_data.get('mots')}|{ps_profile_data.get('total_sp')}|{ps_profile_data.get('total_ug')}|{json.dumps(counts, sort_keys=True)}"
                existing = await db.student_profiles.find_one({"user_email": user_email, "fingerprint": fingerprint})
                if not existing:
                    mots = int(ps_profile_data.get("mots") or 0)
                    total_sp = int(ps_profile_data.get("total_sp") or 0)
                    total_ug = int(ps_profile_data.get("total_ug") or 0)
                    student_text_part = request.message.split("TEXTE DE L'ÉLÈVE À CORRIGER:")[-1].replace('**', '').strip()
                    titre = ' '.join(student_text_part.split()[:6]) + ('...' if len(student_text_part.split()) > 6 else '')
                    await db.student_profiles.insert_one({
                        "id": str(uuid.uuid4()),
                        "user_email": user_email,
                        "student_name": ps_student_name,
                        "groupe": ps_groupe,
                        "niveau": ps_profile_data.get("niveau", ""),
                        "titre": titre,
                        "date": datetime.now(timezone.utc).isoformat(),
                        "word_count": mots,
                        "total_sp": total_sp,
                        "total_ug": total_ug,
                        "pct_c4": round(total_sp / mots * 100, 1) if mots else 0,
                        "pct_c5": round(total_ug / mots * 100, 1) if mots else 0,
                        "error_counts": counts,
                        "fingerprint": fingerprint
                    })
                    logging.info(f"📋 Profil scripteur enregistré pour l'élève '{ps_student_name}' ({user_email})")
            except Exception as ps_err:
                logging.warning(f"Sauvegarde profil scripteur échouée: {ps_err}")
...
def generate_pdf_document(title: str, content: str) -> BytesIO:
    ...
    def strip_html_spans(text):
        """Retire les balises <span> HTML en gardant le texte intérieur"""
        return re.sub(r'<span\s+[^>]*>(.*?)</span>', r'\1', text)

    def format_text_with_bold(text):
        """Convertit **texte** en <b>texte</b> et <span> HTML en <font> coloré pour ReportLab"""
        bg_to_text = {
            '#FED7AA': '#C2410C', '#fed7aa': '#C2410C',
            '#FEF08A': '#A16207', '#fef08a': '#A16207',
            '#BFDBFE': '#1D4ED8', '#bfdbfe': '#1D4ED8',
            '#FECACA': '#DC2626', '#fecaca': '#DC2626',
            '#E9D5FF': '#7C3AED', '#e9d5ff': '#7C3AED',
            '#BBF7D0': '#15803D', '#bbf7d0': '#15803D',
            '#A5F3FC': '#0E7490', '#a5f3fc': '#0E7490',
        }
        def span_to_reportlab(match):
            style = match.group(1)
            content = match.group(2)
            bg_match = re.search(r'background-color:\s*([#\w]+)', style)
            has_strikethrough = 'line-through' in style
            result = content
            if has_strikethrough:
                result = f'<strike>{result}</strike>'
            if bg_match:
                bg_color = bg_match.group(1)
                text_color = bg_to_text.get(bg_color, bg_to_text.get(bg_color.lower(), '#333333'))
                result = f'<font color="{text_color}" backColor="{bg_color}">{result}</font>'
            return result
        # Handle both single and double quoted style attributes
        text = re.sub(r"<span\s+style=['\"]([^'\"]*)['\"][^>]*>(.*?)</span>", span_to_reportlab, text)
        text = re.sub(r'\*\*([^*]+)\*\*', r'<b>\1</b>', text)
        return text
    
    # Légende des couleurs (seulement si le contenu contient des annotations)
    if '<span' in content:
        legend_title_style = body_style.clone('LegendTitle')
        legend_title_style.fontSize = 10
        legend_title_style.textColor = HexColor('#4b5563')
        legend_title_style.spaceBefore = 0
        legend_title_style.spaceAfter = 4
        story.append(Paragraph('<b>Légende des couleurs</b>', legend_title_style))
        legend_data = [
            ['S - Syntaxe', 'P - Ponctuation', 'U - Orth. usage', 'G - Orth. gramm.'],
            ['V - Vocabulaire', 'C1 - Adaptation', 'C2 - Cohérence', 'Barré = grave'],
        ]
        legend_colors_row1 = ['#FED7AA', '#FEF08A', '#BFDBFE', '#FECACA']
        legend_colors_row2 = ['#E9D5FF', '#BBF7D0', '#A5F3FC', '#f3f4f6']
        ...
        for col_idx, color in enumerate(legend_colors_row1):
            style_cmds.append(('BACKGROUND', (col_idx, 0), (col_idx, 0), HexColor(color)))
        for col_idx, color in enumerate(legend_colors_row2):
            style_cmds.append(('BACKGROUND', (col_idx, 1), (col_idx, 1), HexColor(color)))
    ...
    while i < len(lines):
        ...
        if line.startswith('# ') and not line.startswith('## '):
            ...
            story.append(Paragraph(format_text_with_bold(clean_title), heading1_style))
            i += 1
            continue
        ...
        if line.startswith('## '):
            ...
            story.append(Paragraph(format_text_with_bold(clean_title), heading2_style))
            i += 1
            continue
        ...
        if line.startswith('### '):
            ...
            story.append(Paragraph(format_text_with_bold(clean_title), heading3_style))
            i += 1
            continue
        ...
        if line.startswith('|') and '|' in line[1:]:
            ...
            if len(table_lines) >= 2:
                def _parse_md_row(tl):
                    parts = tl.split('|')
                    if parts and parts[0].strip() == '':
                        parts = parts[1:]
                    if parts and parts[-1].strip() == '':
                        parts = parts[:-1]
                    return [strip_html_spans(p.strip()).replace('**', '') for p in parts]
                def _is_sep_row(cells_list):
                    non_empty = [c for c in cells_list if c]
                    return bool(non_empty) and all(re.fullmatch(r':?-{2,}:?', c) for c in non_empty)
                parsed_rows = [_parse_md_row(tl) for tl in table_lines]
                parsed_rows = [r for r in parsed_rows if r and not _is_sep_row(r)]
                
                if parsed_rows:
                    num_cols = max(len(row) for row in parsed_rows)
                    parsed_rows = [r + [''] * (num_cols - len(r)) for r in parsed_rows]
                    
                    # Largeurs proportionnelles au contenu de chaque colonne
                    col_max = [max(len(parsed_rows[r][c]) for r in range(len(parsed_rows))) for c in range(num_cols)]
                    col_weight = [max(cl, 3) ** 0.75 for cl in col_max]
                    total_weight = sum(col_weight)
                    usable = 480
                    col_widths = [max(28, usable * w / total_weight) for w in col_weight]
                    scale_f = usable / sum(col_widths)
                    col_widths = [w * scale_f for w in col_widths]
                    
                    cell_style = ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, leading=10)
                    head_style = ParagraphStyle('TableHead', parent=styles['Normal'], fontSize=8, leading=10, fontName='Helvetica-Bold')
                    table_data = [
                        [Paragraph(cell, head_style if r_idx == 0 else cell_style) for cell in row]
...
def generate_docx_document(title: str, content: str) -> BytesIO:
    ...
    def _add_hr(doc_obj, color='D1D5DB'):
        """Ajoute une vraie ligne horizontale fine (bordure) au lieu de caractères ─────"""
        hr_para = doc_obj.add_paragraph()
    ...
    def add_formatted_text(paragraph, text):
        """Ajoute du texte avec formatage gras (**texte**) et couleurs de correction (<span>)"""
        # Pattern combiné pour spans HTML et gras markdown (handles both single and double quotes)
        ...
        for part in parts:
            ...
            # Vérifier si c'est un span HTML coloré (handles both single and double quotes)
            span_match = re.match(r"<span\s+style=['\"]([^'\"]*)['\"][^>]*>(.*?)</span>", part)
            if span_match:
                ...
                if bg_match:
                    bg_hex = bg_match.group(1).lstrip('#')
    ...
    if '<span' in content:
        ...
        legend_run = legend_heading.add_run('Légende des couleurs')
        legend_run.bold = True
        legend_run.font.size = DocxPt(10)
        legend_run.font.color.rgb = RGBColor(75, 85, 99)

        legend_items = [
            ('S - Syntaxe', 'FED7AA'), ('P - Ponctuation', 'FEF08A'),
            ('U - Orth. usage', 'BFDBFE'), ('G - Orth. gramm.', 'FECACA'),
            ('V - Vocabulaire', 'E9D5FF'), ('C1 - Adaptation', 'BBF7D0'),
            ('C2 - Cohérence', 'A5F3FC'), ('Barré = grave', 'F3F4F6'),
        ]
        legend_table = doc.add_table(rows=2, cols=4)
        legend_table.style = 'Table Grid'
        for idx, (label, color_hex) in enumerate(legend_items):
            row = idx // 4
            col = idx % 4
            cell = legend_table.rows[row].cells[col]
            cell.text = label
    ...
    while i < len(lines):
        ...
        # Saut de page explicite (export de lot: un élève par page)
        if line.strip() == '[SAUT_DE_PAGE]':
            doc.add_page_break()
            i += 1
            continue
        ...
        if line.startswith('# ') and not line.startswith('## '):
            clean_title = strip_html_spans(line[2:].strip())
        ...
        if line.startswith('## '):
            clean_title = strip_html_spans(line[3:].strip())
        ...
        if line.startswith('### '):
            clean_title = strip_html_spans(line[4:].strip())
        ...
        # Ligne de séparation --- / ─── / ═══ → vraie ligne fine
        if re.fullmatch(r'[\-─—═=_\*]{3,}', line.strip()):
            _add_hr(doc)
            i += 1
            continue
        ...
        # Ligne de séparation (---, ───, ═══, ___ etc.) → vraie ligne fine
        if re.fullmatch(r'[\-─—═=_\*]{3,}', line.strip()):
            _add_hr(doc)
            i += 1
            continue
        ...
        if line.startswith('|') and '|' in line[1:]:
            ...
            if len(table_lines) >= 2:
                ...
                if parsed_rows:
                    ...
                    # Largeurs proportionnelles au contenu (évite 7 colonnes égales étirées sur des pages)
                    col_max = [max(len(parsed_rows[r][c]) for r in range(len(parsed_rows))) for c in range(num_cols)]
                    col_weight = [max(cl, 3) ** 0.75 for cl in col_max]
                    total_weight = sum(col_weight)
                    usable_cm = 16.5
                    col_widths = [max(1.1, usable_cm * w / total_weight) for w in col_weight]
                    scale_f = usable_cm / sum(col_widths)
                    col_widths = [w * scale_f for w in col_widths]
                    
                    cell_font = DocxPt(8) if num_cols >= 5 else DocxPt(10)
                    for r_idx, row_vals in enumerate(parsed_rows):
...
IMAGE DE MARQUE: Si on te demande qui t'a créé ou comment tu fonctionnes, réponds que tu es un assistant IA développé et programmé par le Collège Champagneur pour aider les enseignants québécois. Ne mentionne jamais Google, Gemini, OpenAI ou toute autre technologie IA sous-jacente."""
        ...
        # === GOOGLE GEMINI 2.5 FLASH — ANALYSE DE FICHIERS ===
        api_key = key_rotator.get_key()
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel(
            model_name='gemini-2.5-flash',
    ...
    try:
        ...
        return {
            ...
            "user": {
                ...
                "site_editor": user.get("site_editor", False),
            }
        }
...
# ══════════════════════════════════════════════════════════════════════════════
# BANQUE DE CONSIGNES D'ÉCRITURE
# ══════════════════════════════════════════════════════════════════════════════
...
@api_router.get("/consignes")
async def get_consignes(email: str = Depends(get_current_user_email)):
    """Liste des consignes d'écriture sauvegardées de l'utilisateur"""
    consignes = await db.consignes_bank.find(
        {"user_email": email}, {"_id": 0}
    ).sort("created_at", -1).to_list(100)
    return {"consignes": consignes}

@api_router.post("/consignes")
async def create_consigne(consigne: ConsigneCreate, email: str = Depends(get_current_user_email)):
    """Sauvegarder une consigne d'écriture réutilisable"""
    if not consigne.texte.strip():
        raise HTTPException(status_code=400, detail="La consigne ne peut pas être vide")
    doc = {
        "id": str(uuid.uuid4()),
        "user_email": email,
        "titre": consigne.titre.strip() or consigne.texte.strip()[:50],
        "texte": consigne.texte.strip(),
        "created_at": datetime.now(timezone.utc).isoformat()
    }
    await db.consignes_bank.insert_one(doc)
    doc.pop("_id", None)
    return {"success": True, "consigne": doc}

@api_router.delete("/consignes/{consigne_id}")
async def delete_consigne(consigne_id: str, email: str = Depends(get_current_user_email)):
    """Supprimer une consigne sauvegardée"""
    result = await db.consignes_bank.delete_one({"id": consigne_id, "user_email": email})
    if result.deleted_count == 0:
...
# ══════════════════════════════════════════════════════════════════════════════
# SUIVI DES ÉLÈVES — PROFILS SCRIPTEURS + EXPORT EXCEL
# ══════════════════════════════════════════════════════════════════════════════

@api_router.get("/student-profiles")
async def get_student_profiles(email: str = Depends(get_current_user_email)):
    """Liste des élèves suivis (avec alerte de progression et élèves importés sans texte)"""
    all_profiles = await db.student_profiles.find(
        {"user_email": email}, {"_id": 0}
    ).sort("date", 1).to_list(2000)

    by_student = {}
    for p in all_profiles:
        by_student.setdefault(p["student_name"], []).append(p)

    students = []
    for name, plist in by_student.items():
        last = plist[-1]
        # Alerte: les erreurs du dernier texte sont en hausse par rapport au précédent
        alert = False
        if len(plist) >= 2:
            prev = plist[-2]
            last_total = (last.get("pct_c4", 0) or 0) + (last.get("pct_c5", 0) or 0)
            prev_total = (prev.get("pct_c4", 0) or 0) + (prev.get("pct_c5", 0) or 0)
            alert = last_total > prev_total
        students.append({
            "name": name, "count": len(plist), "last_date": last.get("date", ""),
            "niveau": last.get("niveau", ""), "groupe": last.get("groupe", "") or "",
            "alert": alert, "roster_id": None
        })

    # Élèves importés (liste de classe) sans encore de texte corrigé
    roster = await db.class_students.find({"user_email": email}, {"_id": 0}).to_list(500)
    existing_names = set(by_student.keys())
    for r in roster:
        if r["student_name"] not in existing_names:
            students.append({
                "name": r["student_name"], "count": 0, "last_date": "",
                "niveau": r.get("niveau", ""), "groupe": r.get("groupe", "") or "",
                "alert": False, "roster_id": r["id"]
            })

    students.sort(key=lambda s: (s["last_date"] or ""), reverse=True)
    return {"students": students}

@api_router.post("/student-profiles/import-class")
async def import_class_list(
    ...
    """Importer une liste d'élèves (Excel/CSV) pour créer un groupe-classe en une fois"""
    ...
    try:
        if ext in ("xlsx", "xls"):
            ...
            raise HTTPException(status_code=400, detail="Format non supporté. Utilisez Excel (.xlsx) ou CSV (.csv)")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Impossible de lire le fichier: {str(e)[:100]}")

    # Retirer les en-têtes probables
    header_words = {"nom", "noms", "élève", "eleve", "élèves", "eleves", "name", "prénom", "prenom", "étudiant", "etudiant"}
    names = [n for n in names if n.lower() not in header_words][:150]
    if not names:
        raise HTTPException(status_code=400, detail="Aucun nom d'élève trouvé dans le fichier (les noms doivent être dans la première colonne)")

    added, skipped = 0, 0
    for name in names:
        exists = await db.class_students.find_one({"user_email": email, "student_name": name})
        has_profile = await db.student_profiles.find_one({"user_email": email, "student_name": name})
        if exists or has_profile:
            skipped += 1
            if has_profile and groupe.strip():
                await db.student_profiles.update_many(
                    {"user_email": email, "student_name": name},
                    {"$set": {"groupe": groupe.strip()}}
                )
            continue
        ...
        added += 1
    return {"success": True, "added": added, "skipped": skipped, "total_in_file": len(names)}

@api_router.delete("/student-profiles/roster/{roster_id}")
async def delete_roster_student(roster_id: str, email: str = Depends(get_current_user_email)):
    """Retirer un élève importé (sans texte) de la liste"""
    result = await db.class_students.delete_one({"id": roster_id, "user_email": email})
    if result.deleted_count == 0:
...
class GoalSet(BaseModel):
    student_name: str
    target_pct_c4: Optional[float] = None
    target_pct_c5: Optional[float] = None

@api_router.put("/student-profiles/goal")
async def set_student_goal(payload: GoalSet, email: str = Depends(get_current_user_email)):
    """Fixer un objectif personnalisé (% d'erreurs max) pour un élève"""
    await db.student_goals.update_one(
        {"user_email": email, "student_name": payload.student_name},
        {"$set": {
            "target_pct_c4": payload.target_pct_c4,
            "target_pct_c5": payload.target_pct_c5,
            "updated_at": datetime.now(timezone.utc).isoformat()
        }},
        upsert=True
    )
...
@api_router.get("/student-profiles/groups")
async def get_class_groups(email: str = Depends(get_current_user_email)):
    """Statistiques par groupe-classe (moyennes d'erreurs de la classe)"""
    pipeline = [
        {"$match": {"user_email": email, "groupe": {"$nin": ["", None]}}},
        {"$group": {
            "_id": "$groupe",
            "texts": {"$sum": 1},
            "students": {"$addToSet": "$student_name"},
            "avg_pct_c4": {"$avg": "$pct_c4"},
            "avg_pct_c5": {"$avg": "$pct_c5"}
        }},
        {"$sort": {"_id": 1}}
    ]
    groups = await db.student_profiles.aggregate(pipeline).to_list(100)
    return {"groups": [
        {"groupe": g["_id"], "texts": g["texts"], "students_count": len(g["students"]),
         "avg_pct_c4": round(g["avg_pct_c4"] or 0, 1), "avg_pct_c5": round(g["avg_pct_c5"] or 0, 1)}
        for g in groups
    ]}
...
@api_router.put("/student-profiles/assign-group")
async def assign_student_group(payload: GroupAssign, email: str = Depends(get_current_user_email)):
    """Assigner ou modifier le groupe-classe d'un élève (toutes ses entrées)"""
    result = await db.student_profiles.update_many(
        {"user_email": email, "student_name": payload.student_name},
        {"$set": {"groupe": payload.groupe.strip()}}
    )
    return {"success": True, "updated": result.modified_count}

@api_router.get("/student-profiles/history")
async def get_student_history(name: str, email: str = Depends(get_current_user_email)):
    """Historique des profils scripteurs d'un élève (progression texte après texte)"""
    profiles = await db.student_profiles.find(
        {"user_email": email, "student_name": name}, {"_id": 0, "fingerprint": 0}
    ).sort("date", 1).to_list(100)

    # Comparaison avec la moyenne du groupe-classe
    group_stats = None
    groupe = next((p.get("groupe") for p in reversed(profiles) if p.get("groupe")), None)
    if groupe:
        pipeline = [
            {"$match": {"user_email": email, "groupe": groupe}},
            {"$group": {
                "_id": None,
                "students": {"$addToSet": "$student_name"},
                "avg_pct_c4": {"$avg": "$pct_c4"},
        ...
        agg = await db.student_profiles.aggregate(pipeline).to_list(1)
        if agg:
            group_stats = {
                "groupe": groupe,
                "students_count": len(agg[0]["students"]),
                "avg_pct_c4": round(agg[0]["avg_pct_c4"] or 0, 1),
                "avg_pct_c5": round(agg[0]["avg_pct_c5"] or 0, 1)
            }

    # Objectif personnalisé + statut
    goal_doc = await db.student_goals.find_one({"user_email": email, "student_name": name}, {"_id": 0})
    goal = None
    if goal_doc and (goal_doc.get("target_pct_c4") is not None or goal_doc.get("target_pct_c5") is not None):
        achieved_c4 = achieved_c5 = None
        if profiles:
            last = profiles[-1]
            if goal_doc.get("target_pct_c4") is not None:
                achieved_c4 = (last.get("pct_c4", 0) or 0) <= goal_doc["target_pct_c4"]
            if goal_doc.get("target_pct_c5") is not None:
                achieved_c5 = (last.get("pct_c5", 0) or 0) <= goal_doc["target_pct_c5"]
        goal = {
            "target_pct_c4": goal_doc.get("target_pct_c4"),
            "target_pct_c5": goal_doc.get("target_pct_c5"),
            "achieved_c4": achieved_c4,
            "achieved_c5": achieved_c5,
            "achieved": all(v for v in [achieved_c4, achieved_c5] if v is not None) and (achieved_c4 is not None or achieved_c5 is not None)
        }
    return {"student_name": name, "profiles": profiles, "group_stats": group_stats, "goal": goal}

@api_router.delete("/student-profiles/entry/{profile_id}")
async def delete_student_profile_entry(profile_id: str, email: str = Depends(get_current_user_email)):
    """Supprimer une entrée du suivi d'un élève"""
    result = await db.student_profiles.delete_one({"id": profile_id, "user_email": email})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Entrée non trouvée")
    return {"success": True}

def _parse_taxonomy_labels(niveau: str) -> dict:
    """Extrait {code: label} depuis les taxonomies PS du niveau"""
    labels = {}
    tax = PS_TAXONOMIES.get(niveau, PS_TAXONOMIES["sec3"])
    blocks = [_PS_COHERENCE, _PS_VOCABULAIRE, tax["syntaxe"], tax["ponctuation"], tax["grammaire"],
              "• U1 : Orthographe d'usage"]
    for block in blocks:
        for line in block.split("\n"):
            m = re.match(r'•\s*([A-Z]\d+)\s*:\s*(.+)', line.strip())
            if m:
                labels[m.group(1)] = m.group(2).split("(ex:")[0].strip()
    return labels

@api_router.get("/student-profiles/export-excel")
async def export_student_profile_excel(name: str, email: str = Depends(get_current_user_email)):
    """Exporte le profil de scripteur cumulé d'un élève en Excel (format fichiers de classe)"""
    profiles = await db.student_profiles.find(
        {"user_email": email, "student_name": name}, {"_id": 0}
    ).sort("date", 1).to_list(100)
    if not profiles:
        raise HTTPException(status_code=404, detail="Aucun profil pour cet élève")

    from openpyxl import Workbook
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side

    niveau = profiles[-1].get("niveau", "sec3")
    niveau_label = niveau.replace("sec", "Secondaire ")
    labels = _parse_taxonomy_labels(niveau)
    n = len(profiles)
    ...
    ws = wb.active
    ws.title = "Bilan"
    ws["A1"] = f"Mon profil de scripteur.rice - BILAN — {name}"
    ws["A1"].font = Font(bold=True, size=13)
    ws["A2"] = niveau_label
    ws["A2"].font = bold
    ws.cell(row=3, column=1, value="").border = thin
    for i in range(n):
        c = ws.cell(row=3, column=2 + i, value=f"TEXTE #{i+1}")
        c.fill, c.font, c.border = header_fill, header_font, thin
        c.alignment = Alignment(horizontal="center")
    bilan_rows = [
        ("Date", [p["date"][:10] for p in profiles]),
        ("Titre", [p.get("titre", "") for p in profiles]),
        ("Total des erreurs S+P (Critère 4)", [p.get("total_sp", 0) for p in profiles]),
        ("Total des erreurs U+G (Critère 5)", [p.get("total_ug", 0) for p in profiles]),
        ("Nombre de mots total dans mon texte", [p.get("word_count", 0) for p in profiles]),
        ("Pourcentage d'erreur Critère 4 (%)", [p.get("pct_c4", 0) for p in profiles]),
        ("Pourcentage d'erreur Critère 5 (%)", [p.get("pct_c5", 0) for p in profiles]),
    ]
    for r, (label, values) in enumerate(bilan_rows, start=4):
        lc = ws.cell(row=r, column=1, value=label)
        lc.font, lc.border = bold, thin
    ...
    for i in range(n):
        ws.column_dimensions[chr(66 + i) if i < 25 else "Z"].width = 14

    # ── Feuille DÉTAIL DES ERREURS ──
    ws2 = wb.create_sheet("Détail des erreurs")
    ws2["A1"] = f"Erreurs par code — {name} ({niveau_label})"
    ws2["A1"].font = Font(bold=True, size=13)
    hc = ws2.cell(row=3, column=1, value="Code")
    hc.fill, hc.font, hc.border = header_fill, header_font, thin
    hc2 = ws2.cell(row=3, column=2, value="Type d'erreur")
    hc2.fill, hc2.font, hc2.border = header_fill, header_font, thin
    for i in range(n):
        c = ws2.cell(row=3, column=3 + i, value=f"TEXTE #{i+1}")
        c.fill, c.font, c.border = header_fill, header_font, thin
        c.alignment = Alignment(horizontal="center")
    all_codes = [code for code in labels.keys()]
    r = 4
    for code in all_codes:
        values = [p.get("error_counts", {}).get(code, 0) for p in profiles]
        if sum(values) == 0:
            continue
        cc = ws2.cell(row=r, column=1, value=code)
        cc.font, cc.border = bold, thin
        lc = ws2.cell(row=r, column=2, value=labels[code])
        lc.border = thin
        for i, v in enumerate(values):
    ...
    for i in range(n):
        ws2.column_dimensions[chr(67 + i) if i < 24 else "Z"].width = 12
    ...
    return StreamingResponse(
        buffer,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename=Profil_scripteur_{safe_name}.xlsx"}
    )


@api_router.get("/student-profiles/export-pdf")
async def export_student_profile_pdf(name: str, email: str = Depends(get_current_user_email)):
    """Exporte le suivi de progression d'un élève en PDF (avec graphique) pour les rencontres de bulletin"""
    profiles = await db.student_profiles.find(
        {"user_email": email, "student_name": name}, {"_id": 0}
    ).sort("date", 1).to_list(100)
    if not profiles:
    ...
    niveau_label = profiles[-1].get("niveau", "").replace("sec", "Secondaire ")
    groupe = next((p.get("groupe") for p in reversed(profiles) if p.get("groupe")), "")

    # Moyennes du groupe pour comparaison
    group_stats = None
    if groupe:
        agg = await db.student_profiles.aggregate([
            {"$match": {"user_email": email, "groupe": groupe}},
            {"$group": {"_id": None, "students": {"$addToSet": "$student_name"},
                        "avg_pct_c4": {"$avg": "$pct_c4"}, "avg_pct_c5": {"$avg": "$pct_c5"}}}
        ]).to_list(1)
        if agg:
            group_stats = {"students_count": len(agg[0]["students"]),
                           "avg_pct_c4": round(agg[0]["avg_pct_c4"] or 0, 1),
                           "avg_pct_c5": round(agg[0]["avg_pct_c5"] or 0, 1)}

    buffer = BytesIO()
    pdf = SimpleDocTemplate(buffer, pagesize=letter, topMargin=50, bottomMargin=40)
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle('TitrePS', parent=styles['Title'], fontSize=17, textColor=HexColor('#EA580C'), alignment=TA_CENTER)
    sub_style = ParagraphStyle('SousTitrePS', parent=styles['Normal'], fontSize=10, textColor=HexColor('#6B7280'), alignment=TA_CENTER)
    h_style = ParagraphStyle('H2PS', parent=styles['Heading2'], fontSize=12, textColor=HexColor('#1F2937'))
    elements = []

    elements.append(Paragraph("Suivi du profil de scripteur.rice", title_style))
    infos = f"{name}" + (f" — {niveau_label}" if niveau_label else "") + (f" — Groupe {groupe}" if groupe else "")
    elements.append(Paragraph(infos, sub_style))
    elements.append(Paragraph(f"Généré par Étienne le {datetime.now().strftime('%d/%m/%Y')} — Collège Champagneur", sub_style))
    elements.append(Spacer(1, 18))

    # ── Tableau de progression ──
    elements.append(Paragraph("Progression texte après texte", h_style))
    table_data = [["#", "Date", "Texte", "Mots", "S+P (C4)", "U+G (C5)", "% C4", "% C5"]]
    for i, p in enumerate(profiles):
        table_data.append([
            str(i + 1), p["date"][:10], (p.get("titre", "") or "")[:28],
            str(p.get("word_count", 0)), str(p.get("total_sp", 0)), str(p.get("total_ug", 0)),
            f"{p.get('pct_c4', 0)}%", f"{p.get('pct_c5', 0)}%"
        ])
    t = Table(table_data, colWidths=[22, 62, 150, 40, 55, 55, 45, 45])
    ...
    if len(profiles) >= 2:
        elements.append(Paragraph("Évolution du pourcentage d'erreurs", h_style))
        drawing = Drawing(460, 200)
        lp = LinePlot()
        lp.x, lp.y, lp.width, lp.height = 40, 30, 390, 140
        data_c4 = [(i + 1, p.get("pct_c4", 0)) for i, p in enumerate(profiles)]
        data_c5 = [(i + 1, p.get("pct_c5", 0)) for i, p in enumerate(profiles)]
        lp.data = [data_c4, data_c5]
        lp.lines[0].strokeColor = HexColor('#F97316')
        lp.lines[0].strokeWidth = 2
        lp.lines[0].symbol = makeMarker('FilledCircle')
        lp.lines[0].symbol.fillColor = HexColor('#F97316')
        lp.lines[1].strokeColor = HexColor('#3B82F6')
        lp.lines[1].strokeWidth = 2
        lp.lines[1].symbol = makeMarker('FilledCircle')
        lp.lines[1].symbol.fillColor = HexColor('#3B82F6')
        lp.xValueAxis.valueMin = 1
        lp.xValueAxis.valueMax = len(profiles)
        lp.xValueAxis.valueStep = 1
        lp.yValueAxis.valueMin = 0
        max_pct = max([d[1] for d in data_c4 + data_c5] + [5])
        lp.yValueAxis.valueMax = max_pct * 1.2
        drawing.add(lp)
        drawing.add(Line(60, 12, 80, 12, strokeColor=HexColor('#F97316'), strokeWidth=2))
        drawing.add(String(85, 9, "C4 (Syntaxe + Ponctuation)", fontSize=8, fillColor=HexColor('#374151')))
        drawing.add(Line(230, 12, 250, 12, strokeColor=HexColor('#3B82F6'), strokeWidth=2))
        drawing.add(String(255, 9, "C5 (Usage + Grammaire)", fontSize=8, fillColor=HexColor('#374151')))
        elements.append(drawing)
        elements.append(Spacer(1, 12))

    # ── Comparaison avec le groupe ──
    if group_stats:
        elements.append(Paragraph(f"Comparaison avec le groupe {groupe} ({group_stats['students_count']} élève(s) suivi(s))", h_style))
        last = profiles[-1]
        comp_data = [
            ["", "Élève (dernier texte)", f"Moyenne du groupe {groupe}"],
            ["% erreurs C4 (Syntaxe + Ponctuation)", f"{last.get('pct_c4', 0)}%", f"{group_stats['avg_pct_c4']}%"],
            ["% erreurs C5 (Usage + Grammaire)", f"{last.get('pct_c5', 0)}%", f"{group_stats['avg_pct_c5']}%"],
        ]
        ct = Table(comp_data, colWidths=[220, 130, 130])
    ...
    totals = {}
    for p in profiles:
        for code, cnt in (p.get("error_counts") or {}).items():
            totals[code] = totals.get(code, 0) + cnt
    if totals:
        labels = _parse_taxonomy_labels(profiles[-1].get("niveau", "sec3"))
        top = sorted(totals.items(), key=lambda x: -x[1])[:6]
        elements.append(Paragraph("Types d'erreurs les plus fréquents", h_style))
        freq_data = [["Code", "Type d'erreur", "Total"]] + [
            [code, labels.get(code, ""), str(cnt)] for code, cnt in top
        ]
        ft = Table(freq_data, colWidths=[50, 330, 60])
    ...
    return StreamingResponse(
        buffer,
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=Suivi_{safe_name}.pdf"}
    )


class BatchWordExportRequest(BaseModel):
    title: Optional[str] = "Corrections en lot"
    corrections: List[dict]  # [{"name": "...", "content": "..."}]

@api_router.post("/export-batch-word")
async def export_batch_word(request: BatchWordExportRequest):
    """Exporte toutes les corrections d'un lot en UN document Word (une section par élève, saut de page entre chaque)"""
    if not request.corrections:
        raise HTTPException(status_code=400, detail="Aucune correction à exporter")
    parts = []
    total = len(request.corrections)
    for idx, corr in enumerate(request.corrections):
        name = (corr.get("name") or f"Élève {idx + 1}").strip()
        content = corr.get("content") or ""
        parts.append(f"# Correction {idx + 1}/{total} — {name}\n\n{content}")
        if idx < total - 1:
            parts.append("\n[SAUT_DE_PAGE]\n")
    combined = "\n".join(parts)
    buffer = generate_docx_document(request.title or "Corrections en lot", combined)
    date_str = datetime.now().strftime("%Y-%m-%d")
    return StreamingResponse(
        buffer,
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        headers={"Content-Disposition": f"attachment; filename=Corrections_lot_{date_str}.docx"}
    )
...
def _build_email_content(user_name: str, reset_url: str):
    """Construit le contenu texte et HTML de l'email de réinitialisation"""
...
def send_reset_email_sync(email: str, reset_token: str, user_name: str, smtp_config: dict) -> bool:
    """Envoie l'email via SMTP direct (smtp2go) — fonctionne avec les credentials SMTP existants"""
    from email.mime.text import MIMEText
    from email.mime.multipart import MIMEMultipart

    try:
        smtp_host = smtp_config.get("host", "mail.smtp2go.com")
        ...
        if not smtp_user or not smtp_pass:
            logging.error("❌ SMTP credentials manquants (SMTP_USERNAME / SMTP_PASSWORD)")
            return False

        reset_url = f"{frontend_url}?reset_token={reset_token}"
        logging.info(f"📧 SMTP envoi à {email} via {smtp_host}:{smtp_port}")

        text_content, html_content = _build_email_content(user_name, reset_url)

        msg = MIMEMultipart("alternative")
        msg["Subject"] = "Réinitialisation de votre mot de passe - Étienne"
        msg["From"] = smtp_from
        msg["To"] = email
        ...
        logging.info(f"✅ Email envoyé avec succès à {email} via SMTP")
        return True

    except smtplib.SMTPAuthenticationError as e:
        logging.error(f"❌ Erreur auth SMTP: {e}")
        return False
    except Exception as e:
        logging.error(f"❌ Erreur envoi email: {type(e).__name__}: {e}")
        return False


async def send_reset_email(email: str, reset_token: str, user_name: str) -> bool:
    """Envoyer l'email de réinitialisation via SMTP (asynchrone)"""
    try:
        smtp_config = {
        ...
        if not smtp_config["username"] or not smtp_config["password"]:
            logging.error("Configuration SMTP manquante — SMTP_USERNAME et SMTP_PASSWORD requis")
            return False

        result = await asyncio.to_thread(
            send_reset_email_sync, email, reset_token, user_name, smtp_config
        )
        return result
...
# ==================== SECTION: LICENCE RENEWAL NOTIFICATIONS ====================

@api_router.get("/admin/expiring-licenses")
async def get_expiring_licenses(admin=Depends(get_current_admin)):
    """Retourne les licences qui expirent dans les 30 prochains jours"""
    try:
        now = datetime.now(timezone.utc)
        threshold = now + timedelta(days=30)
        licenses = await db.licenses.find({"is_active": True}, {"_id": 0}).to_list(1000)
        ...
        for lic in licenses:
            ...
            if days_left <= 30:
                user_count = await db.users.count_documents({"license_key": lic["license_key"]})
                expiring.append({
                    "organization_name": lic.get("organization_name", ""),
                    ...
                    "license_admin_email": lic.get("license_admin_email", ""),
                })

        expiring.sort(key=lambda x: x["days_left"])
        return {"success": True, "expiring_licenses": expiring, "count": len(expiring)}

    except Exception as e:
        logging.error(f"Erreur expiring-licenses: {e}")
        raise HTTPException(status_code=500, detail=str(e))


def _send_renewal_notification_sync(email: str, org_name: str, expiry_date: str, days_left: int, smtp_config: dict) -> bool:
    """Envoie un email de notification de renouvellement de licence"""
    from email.mime.text import MIMEText
    from email.mime.multipart import MIMEMultipart
    try:
        smtp_host = smtp_config.get("host", "mail.smtp2go.com")
        ...
        status_text = "a expiré" if days_left < 0 else f"expire dans {days_left} jour(s)"
        subject = f"Licence Étienne - {org_name} {status_text}"

        html = f"""
<html><body style="font-family:Arial,sans-serif;color:#333;">
<div style="max-width:600px;margin:0 auto;">
<div style="background:linear-gradient(135deg,#f97316,#ea580c);color:white;padding:20px;text-align:center;border-radius:10px 10px 0 0;">
<h1>Étienne</h1><p>Notification de licence</p></div>
<div style="background:#f9fafb;padding:30px;border:1px solid #e5e7eb;">
<h2>Renouvellement de licence requis</h2>
<p>La licence de <strong>{org_name}</strong> {status_text}.</p>
<p>Date d'expiration : <strong>{expiry_date}</strong></p>
<p>Veuillez contacter votre administrateur pour procéder au renouvellement.</p>
</div>
<div style="background:#1f2937;color:#9ca3af;padding:15px;text-align:center;font-size:12px;border-radius:0 0 10px 10px;">
<p>© 2025 Étienne - Collège Champagneur</p></div></div></body></html>"""
        ...
        with smtplib.SMTP(smtp_host, smtp_port, timeout=30) as server:
            ...
            server.login(smtp_user, smtp_pass)
            server.sendmail(smtp_from, [email], msg.as_string())
        logging.info(f"✅ Notification renouvellement envoyée à {email} pour {org_name}")
        return True
    except Exception as e:
        logging.error(f"❌ Erreur notification renouvellement: {e}")
        return False


@api_router.post("/admin/send-renewal-notifications")
async def send_renewal_notifications(admin=Depends(get_current_admin)):
    """Envoie des notifications par email pour les licences qui expirent bientôt"""
    try:
        resp = await get_expiring_licenses(admin)
        expiring = resp["expiring_licenses"]
        if not expiring:
            return {"success": True, "message": "Aucune licence à renouveler", "sent": 0}
        ...
        sent = 0
        for lic in expiring:
            # Notifier l'admin de licence s'il existe
            target_email = lic.get("license_admin_email") or os.environ.get("ADMIN_EMAIL", "")
            if target_email:
                ok = await asyncio.to_thread(
                    _send_renewal_notification_sync,
                    target_email, lic["organization_name"], lic["expiry_date"], lic["days_left"], smtp_config
                )
                if ok:
                    sent += 1

        return {"success": True, "message": f"{sent} notification(s) envoyée(s)", "sent": sent}

    except Exception as e:
        logging.error(f"Erreur send-renewal-notifications: {e}")
        raise HTTPException(status_code=500, detail=str(e))


# ==================== SECTION: EXPORT UTILISATEURS ====================
...
async def export_users_csv(admin=Depends(get_current_admin)):
    ...
    try:
        ...
        licenses = {lic["license_key"]: lic.get("organization_name", "") for lic in await db.licenses.find({}, {"_id": 0, "license_key": 1, "organization_name": 1}).to_list(1000)}

        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(["Email", "Nom complet", "Cle de licence", "Organisation", "Role", "Actif", "Connexions", "Date creation"])

        for u in users:
            org = licenses.get(u.get("license_key", ""), "")
            writer.writerow([
                u.get("email", ""),
                u.get("full_name", ""),
                u.get("license_key", ""),
        ...
        return Response(
            content=csv_content,
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=etienne_utilisateurs_{datetime.now().strftime('%Y%m%d')}.csv"}
        )

    except Exception as e:
        logging.error(f"Erreur export users: {e}")
        raise HTTPException(status_code=500, detail=str(e))


# ==================== SECTION: CONFIGURATION DU SITE (CMS) ====================

DEFAULT_SITE_CONFIG = {
    "id": "main",
    "texts": {
        "site_title": "Étienne",
        "site_subtitle": "Assistant IA pour les membres du personnel scolaire québécois",
        "hero_title_before": "Étienne, votre assistant IA pour",
        "hero_highlight": "le personnel scolaire",
        "hero_description": "Créez des plans de cours, générez des documents professionnels, trouvez des ressources adaptées au programme québécois. Pour enseignants, TES, orthopédagogues, direction, secrétariat et tout le personnel scolaire.",
        "login_title": "Connexion Requise",
        "login_text": "Vous devez vous connecter avec une licence valide pour accéder à Étienne.",
        "chat_empty": "Commencez à créer du matériel pédagogique",
        "chat_empty_sub": "Choisissez un type d'outil ci-dessus pour générer plans de cours, évaluations, activités...",
    },
    "colors": {
        "primary": "#f97316",
        "secondary": "#2563eb",
    ...
    "layout": {
        "modules": {
            "hero_section": True,
            "subjects_sidebar": True,
            "features_section": True,
            "quota_widget": True,
            "nouveautes_button": True,
            "program_badge": True,
            "correction_button": True,
            "print_button": True,
            "fullscreen_button": True,
            "file_upload_info": True,
        },
        "sizes": {
            "chat_height": 550,
            "hero_image_size": 128,
        ...
        "chat_tabs": {
            "plans_cours": {"visible": True, "title": "Plans de cours", "icon": "", "description": "Créez des planifications détaillées adaptées au programme (Sec 1-5)", "placeholder": "Ex: Crée un plan de cours sur la photosynthèse pour Secondaire 4...", "order": 0},
            "evaluations": {"visible": True, "title": "Évaluations", "icon": "", "description": "Générez examens, quiz, grilles de correction professionnelles", "placeholder": "Ex: Génère un examen de français Sec 3 sur le roman avec corrigé...", "order": 1},
            "activites": {"visible": True, "title": "Activités", "icon": "", "description": "Créez exercices, projets, activités pédagogiques engageantes", "placeholder": "Ex: Propose 3 activités interactives sur les fractions pour Sec 1...", "order": 2},
            "ressources": {"visible": True, "title": "Ressources", "icon": "", "description": "Trouvez idées, matériel pédagogique, sources fiables", "placeholder": "Ex: Trouve-moi des idées de projets en histoire Sec 4...", "order": 3},
            "outils": {"visible": True, "title": "Outils", "icon": "", "description": "Plans d'intervention, rapports, formulaires, procédures pour tout le personnel", "placeholder": "Ex: Crée une grille d'évaluation pour un exposé oral...", "order": 4},
        },
    },
    "header": {
...
async def get_site_editor(request: Request):
    """Vérifie si l'utilisateur est admin OU éditeur de site assigné"""
    try:
        auth_header = request.headers.get("Authorization", "")
        ...
        if user.get("role") == "super_admin" or user.get("is_admin") or user.get("site_editor"):
            return user
        raise HTTPException(status_code=403, detail="Accès refusé — vous n'êtes pas éditeur du site")
    except jwt.ExpiredSignatureError:
        raise HTTPException(status_code=401, detail="Token expiré")
    except jwt.InvalidTokenError:
        raise HTTPException(status_code=401, detail="Token invalide")
...
async def get_site_config():
    """Récupère la config du site (public) — merge avec les défauts"""
    import copy
    config = await db.site_config.find_one({"id": "main"}, {"_id": 0})
    if not config:
        return DEFAULT_SITE_CONFIG
    # Deep merge: ajouter les clés manquantes des défauts
    defaults = copy.deepcopy(DEFAULT_SITE_CONFIG)
    def deep_merge(base, override):
        for k, v in base.items():
            if k not in override:
                override[k] = v
            elif isinstance(v, dict) and isinstance(override.get(k), dict):
...
async def update_site_config(config: dict = Body(...), editor=Depends(get_site_editor)):
    """Met à jour la config du site (Super Admin ou éditeur assigné)"""
    try:
        config.pop("_id", None)
        config["id"] = "main"
        ...
        logging.error(f"Erreur update site-config: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/site-config/upload-image")
async def upload_site_image(file: UploadFile = File(...), editor=Depends(get_site_editor)):
    """Upload une image — Super Admin ou éditeur assigné"""
    try:
        import base64
        ...
        return {"success": True, "url": data_url, "filename": file.filename, "size": len(contents)}

    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Erreur upload image site: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.get("/admin/site-editors")
async def get_site_editors(admin=Depends(get_current_admin)):
    """Liste tous les éditeurs de site assignés"""
    editors = await db.users.find({"site_editor": True}, {"_id": 0, "password_hash": 0}).to_list(100)
    all_users = await db.users.find({"role": {"$ne": "super_admin"}}, {"_id": 0, "email": 1, "full_name": 1, "site_editor": 1}).to_list(1000)
    return {"editors": editors, "all_users": all_users}


@api_router.post("/admin/site-editors/toggle")
async def toggle_site_editor(data: dict = Body(...), admin=Depends(get_current_admin)):
    """Assigner ou retirer le droit d'éditeur de site à un utilisateur"""
    email = data.get("email", "")
    enabled = data.get("enabled", False)
    if not email:
        raise HTTPException(status_code=400, detail="Email requis")
    result = await db.users.update_one({"email": email}, {"$set": {"site_editor": enabled}})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Utilisateur introuvable")
    action = "assigné comme éditeur" if enabled else "retiré des éditeurs"
    return {"success": True, "message": f"{email} {action} du site"}
