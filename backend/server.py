...
def generate_pdf_document(title: str, content: str) -> BytesIO:
    ...
    def clean_latex_for_export(text):
        """Nettoie le code LaTeX pour l'export PDF/Word - convertit en texte lisible"""
        if not text:
            return ''
        ...
        # Convertir les racines carrées
        cleaned = re.sub(r'\\sqrt\[(\d+)\]\{([^{}]*)\}', r'∜(\2)', cleaned)
        ...
        cleaned = re.sub(r'\^0(?![0-9])', '⁰', cleaned)
        cleaned = re.sub(r'\^{1}', '¹', cleaned)
        cleaned = re.sub(r'\^1(?![0-9])', '¹', cleaned)
        cleaned = re.sub(r'\^{2}', '²', cleaned)
        cleaned = re.sub(r'\^2(?![0-9])', '²', cleaned)
        cleaned = re.sub(r'\^{3}', '³', cleaned)
        cleaned = re.sub(r'\^3(?![0-9])', '³', cleaned)
        cleaned = re.sub(r'\^{4}', '⁴', cleaned)
        cleaned = re.sub(r'\^{5}', '⁵', cleaned)
        cleaned = re.sub(r'\^{6}', '⁶', cleaned)
        cleaned = re.sub(r'\^{7}', '⁷', cleaned)
        cleaned = re.sub(r'\^{8}', '⁸', cleaned)
        cleaned = re.sub(r'\^{9}', '⁹', cleaned)
        cleaned = re.sub(r'\^{n}', 'ⁿ', cleaned)
        cleaned = re.sub(r'\^n(?![a-z])', 'ⁿ', cleaned)
        cleaned = re.sub(r'\^{([^}]*)}', r'^(\1)', cleaned)
        
        # Convertir les indices
        cleaned = re.sub(r'_{0}', '₀', cleaned)
        cleaned = re.sub(r'_0(?![0-9])', '₀', cleaned)
        cleaned = re.sub(r'_{1}', '₁', cleaned)
        cleaned = re.sub(r'_1(?![0-9])', '₁', cleaned)
        cleaned = re.sub(r'_{2}', '₂', cleaned)
        cleaned = re.sub(r'_2(?![0-9])', '₂', cleaned)
        ...
        cleaned = re.sub(r'\\mp', '∓', cleaned)
        cleaned = re.sub(r'\\le(?:q)?', '≤', cleaned)
        cleaned = re.sub(r'\\ge(?:q)?', '≥', cleaned)
        cleaned = re.sub(r'\\neq', '≠', cleaned)
        cleaned = re.sub(r'\\ne(?![a-z])', '≠', cleaned)
        ...
        cleaned = re.sub(r'\\lambda', 'λ', cleaned)
        cleaned = re.sub(r'\\mu', 'μ', cleaned)
        cleaned = re.sub(r'\\sigma', 'σ', cleaned)
        cleaned = re.sub(r'\\omega', 'ω', cleaned)
        cleaned = re.sub(r'\\Delta', 'Δ', cleaned)
        cleaned = re.sub(r'\\Sigma', 'Σ', cleaned)
        cleaned = re.sub(r'\\sum', 'Σ', cleaned)
        cleaned = re.sub(r'\\prod', 'Π', cleaned)
        cleaned = re.sub(r'\\int', '∫', cleaned)
        cleaned = re.sub(r'\\rightarrow', '→', cleaned)
        cleaned = re.sub(r'\\leftarrow', '←', cleaned)
        cleaned = re.sub(r'\\Rightarrow', '⇒', cleaned)
        cleaned = re.sub(r'\\therefore', '∴', cleaned)
        cleaned = re.sub(r'\\angle', '∠', cleaned)
        cleaned = re.sub(r'\\perp', '⊥', cleaned)
        cleaned = re.sub(r'\\parallel', '∥', cleaned)
        cleaned = re.sub(r'\\degree', '°', cleaned)
        ...
        cleaned = re.sub(r'\\big', '', cleaned)
        cleaned = re.sub(r'\\Big', '', cleaned)
        cleaned = re.sub(r'\\cdot', '·', cleaned)
        cleaned = re.sub(r'\\ldots', '...', cleaned)
        cleaned = re.sub(r'\\dots', '...', cleaned)
    ...
    # Nettoyer le contenu LaTeX avant traitement
...
def generate_docx_document(title: str, content: str) -> BytesIO:
    ...
    def clean_latex_for_export(text):
        ...
        # Traiter les fractions imbriquées
        for _ in range(5):
            cleaned = re.sub(r'\\frac\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}', r'(\1/\2)', cleaned)
