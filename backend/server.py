...
[SECTION 8]  RÉPONSE IA (GEMINI)             - Ligne ~1190 (get_ai_response - IMPORTANT)
[SECTION 9]  ROUTES BASIQUES                 - Ligne ~1920 (health, chat, quota)
[SECTION 10] GÉNÉRATION DOCUMENTS            - Ligne ~2090 (PDF, DOCX, PPTX, XLSX)
[SECTION 11] UPLOAD & ANALYSE FICHIERS       - Ligne ~2740
[SECTION 12] MODÈLES PYDANTIC (Admin)        - Ligne ~2910
[SECTION 13] ROUTES DÉTECTION IA             - Ligne ~2990
[SECTION 14] ROUTES ADMIN (Logs, Reports)    - Ligne ~3130
[SECTION 15] GÉNÉRATION IMAGES               - Ligne ~3400 (Hugging Face)
[SECTION 16] AUTHENTIFICATION                - Ligne ~3530 (signup, login)
[SECTION 17] GESTION LICENCES                - Ligne ~3690
[SECTION 18] MOTS BLOQUÉS                    - Ligne ~3900
[SECTION 19] ADMIN MULTI-NIVEAUX             - Ligne ~4020
[SECTION 20] CONVERSATIONS CLOUD             - Ligne ~4310
[SECTION 21] RÉINITIALISATION MOT DE PASSE   - Ligne ~4500
...
def detect_language(text: str) -> str:
    """Detecte la langue du message - PRIORITÉ AU FRANÇAIS pour app québécoise"""
    text_lower = text.lower()
    
    # Indicateurs forts français (prioritaires)
    french_strong = ["bonjour", "salut", "merci", "s'il vous plait", "s'il te plait", "sil vous plait", "sil te plait",
                     "comment vas-tu", "ça va", "ca va", "je suis", "tu es", "nous sommes", "ils sont", "elle est",
                     "je veux", "je voudrais", "peux-tu", "pouvez-vous", "j'aimerais", "j'ai besoin", "aide-moi",
                     "comment faire", "qu'est-ce que", "pourquoi", "comment", "quand", "où est",
                     "pour mes élèves", "pour les élèves", "secondaire", "primaire", "cours de", "leçon",
                     "évaluation", "évaluer", "activité", "exercice", "devoir", "travail pratique"]
    
    # Indicateurs forts anglais - SEULEMENT si vraiment en anglais
    english_strong = ["hello", "hi there", "thank you", "thanks for", "please help", "how are you", 
                      "i am looking", "i need help", "can you help", "could you please", "would you mind",
                      "i want to", "i would like to", "help me with", "show me how", "tell me about",
                      "create a lesson", "generate a quiz", "make an activity", "write a plan"]
    
    # Vérifier les indicateurs forts français d'abord (PRIORITÉ)
    for phrase in french_strong:
        if phrase in text_lower:
            return "fr"
    
    # Vérifier les indicateurs forts anglais ensuite
    for phrase in english_strong:
        if phrase in text_lower:
            return "en"
    
    # Mots EXCLUSIVEMENT anglais (pas de mots qui pourraient être dans un titre)
    english_only_words = [
        "the ", " and ", " you ", " are ", " this ", " that ", " with ", " have ", " from ",
        " help ", " want ", " need ", " about ", " your ", " what ", " when ", " where ",
        " please ", " could ", " would ", " should ", " their ", " these ", " those "
    ]
    ...
    french_words = [
        ...
        " pour ", " dans ", " sur ", " avec ", " par ", " qui ", " que ", " dont ",
        " mon ", " ma ", " mes ", " ton ", " ta ", " tes ", " son ", " sa ", " ses ",
        " notre ", " votre ", " leur ", " faire ", " avoir ", " être ", " aller ",
        " élèves ", " cours ", " classe ", " niveau ", " année ", " secondaire "
    ]
    
    # Compter les mots avec espaces
    text_spaced = f" {text_lower} "
    english_count = sum(1 for word in english_only_words if word in text_spaced)
    french_count = sum(1 for word in french_words if word in text_spaced)
    
    # Par défaut français - il faut BEAUCOUP plus de mots anglais pour basculer
    # (au moins 3 mots anglais de plus que français)
    if english_count > french_count + 3:
        return "en"
    else:
        return "fr"  # DÉFAUT: français pour app québécoise
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ CRITÈRES D'ÉVALUATION EN ÉCRITURE - PFEQ (ORDRE OFFICIEL)                   │
├─────────────────────────────────────────────────────────────────────────────┤
│ CRITÈRE 1: Adaptation à la situation de communication                       │
│   → Respect du destinataire, de l'intention, du contexte                    │
│   → Registre de langue approprié                                            │
│                                                                             │
│ CRITÈRE 2: Cohérence du texte                                               │
│   → Organisation logique des idées                                          │
│   → Progression de l'information                                            │
│   → Reprise de l'information (pronoms, synonymes, etc.)                     │
│   → Utilisation de marqueurs de relation                                    │
│                                                                             │
│ CRITÈRE 3: Utilisation d'un vocabulaire approprié                           │
│   → Précision et variété du vocabulaire                                     │
│   → Termes justes selon le contexte                                         │
│                                                                             │
│ CRITÈRE 4: Construction de phrases et ponctuation appropriées               │
│   → Syntaxe correcte (structure des phrases)                                │
│   → Ponctuation adéquate                                                    │
│   → Variété des types de phrases                                            │
│                                                                             │
│ CRITÈRE 5: Respect des normes relatives à l'orthographe                     │
│   → Orthographe d'usage (lexicale)                                          │
│   → Orthographe grammaticale (accords)                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│ PONDÉRATION SUGGÉRÉE PFEQ (peut être ajustée par l'enseignant):             │
│   • C1 (Adaptation): 10-20%                                                 │
│   • C2 (Cohérence): 20-30%                                                  │
│   • C3 (Vocabulaire): 10-15%                                                │
│   • C4 (Syntaxe/Ponctuation): 15-25%                                        │
│   • C5 (Orthographe): 20-30%                                                │
├─────────────────────────────────────────────────────────────────────────────┤
│ BARÈME DE PÉNALITÉS SUGGÉRÉ (ajustable selon l'enseignant):                 │
│   • Orthographe d'usage: -0.5 pt par faute (ou selon choix enseignant)      │
│   • Orthographe grammaticale: -0.5 à -1 pt par faute                        │
│   • Syntaxe majeure: -1 pt par faute                                        │
│   • Ponctuation: -0.25 à -0.5 pt par faute                                  │
│   • Répétition de la même faute: compter 1 seule fois (selon politique)     │
│                                                                             │
│ ⚠️ IMPORTANT: Toujours demander à l'enseignant s'il veut:                   │
│   1. Utiliser la pondération standard du PFEQ                               │
│   2. Personnaliser les points par type de faute                             │
│   3. Appliquer un plafond de pénalités (ex: max -10 pts pour orthographe)   │
└─────────────────────────────────────────────────────────────────────────────┘
...
"""
        ...
        system_messages = {
            ...
            "evaluations": f"""Tu es Etienne, assistant pour le personnel scolaire quebecois. Genere des evaluations professionnelles (examens, quiz, grilles de correction) conformes au PFEQ.

POUR LES ÉVALUATIONS EN ÉCRITURE (français), utilise TOUJOURS les 5 critères PFEQ dans cet ordre:
...
Si l'utilisateur ne précise pas la pondération des fautes, DEMANDE-LUI s'il veut:
- Utiliser le barème standard PFEQ (ex: -0.5 pt/faute orthographe usage, -1 pt/faute grammaticale)
- Personnaliser les points par type de faute
- Appliquer un plafond de pénalités

Inclus: questions variees, bareme de correction detaille, corrige/solutionnaire.{curriculum_instruction}{branding_instruction_fr}""",
...
async def startup_event():
    """Charge les poids du detecteur au demarrage et initialise les mots bloqués"""
