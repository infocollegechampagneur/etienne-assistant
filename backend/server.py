...
Voulez-vous que j'analyse davantage cette fonction (dérivée, racines, extremums) ?"""
        ...
        system_messages = {
            ...
            "activites": f"Tu es Etienne, createur d activites pedagogiques pour enseignants du secondaire quebecois. Propose des exercices, projets, activites engageantes adaptees au programme. Inclus: consignes claires, materiel necessaire, duree estimee, variantes de differentiation, criteres d evaluation. Genere aussi des exercices avec corriges.{branding_instruction_fr}",
            
            "ressources": f"Tu es Etienne, expert en ressources pedagogiques pour enseignants du secondaire quebecois. Trouve et recommande des sources fiables, idees de projets, materiel didactique, strategies d enseignement. Pour l anglais: Oxford, Cambridge, BBC Learning, Purdue OWL. Pour le Quebec: .gouv.qc.ca, ressources ministerielle.{branding_instruction_fr}",
            
            "outils": f"Tu es Etienne, assistant pour outils pedagogiques des enseignants du secondaire quebecois. Aide avec: strategies de differentiation, grilles d evaluation criteriees, planification (annuelle, mensuelle, hebdomadaire), gestion de classe, adaptations pour eleves en difficulte ou doues. Sois pratique et applicable.{branding_instruction_fr}"
        }
...
Help find resources, explain concepts, clarify information. Cite credible sources.{branding_instruction_en}"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_str = str(e)
                last_error = e
                logging.warning(f"Tentative {attempt + 1}/{max_retries} échouée: {error_str}")
                ...
                if "429" in error_str or "quota" in error_str.lower() or "rate" in error_str.lower():
                    ...
                    if attempt < max_retries - 1:
                        logging.info(f"Attente de {retry_delay}s avant retry...")
                        await asyncio.sleep(min(retry_delay, 30))  # Max 30s d'attente
                    else:
                        return {
                            "response": f"⏳ Quota API dépassé. Le système réessaiera automatiquement dans {retry_delay} secondes.\n\nVeuillez patienter et renvoyer votre message.",
                            "trust_score": None,
                            "sources": [],
                            "quota_exceeded": True,
        ...
        logging.error(f"Erreur IA Gemini après {max_retries} tentatives: {last_error}")
        return {
...
async def send_reset_email(email: str, reset_token: str, user_name: str):
    """Envoyer l'email de réinitialisation via SMTP2GO"""
    try:
        smtp_host = os.environ.get("SMTP_HOST", "mail.smtp2go.com")
        smtp_port = int(os.environ.get("SMTP_PORT", "587"))
        smtp_user = os.environ.get("SMTP_USERNAME", "")
        smtp_password = os.environ.get("SMTP_PASSWORD", "")
        smtp_from = os.environ.get("SMTP_FROM_EMAIL", "noreply@champagneur.qc.ca")
        ...
        frontend_url = os.environ.get("FRONTEND_URL", "https://classroom-ai-12.preview.emergentagent.com")
