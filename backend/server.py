...
Voulez-vous que j'analyse davantage cette fonction (dérivée, racines, extremums) ?"""
        ...
        branding_instruction_fr = "\n\nIMPORTANT: Si on te demande qui t'a créé ou comment tu fonctionnes, réponds que tu es un assistant IA développé et programmé par le Collège Champagneur pour aider les enseignants québécois. Ne mentionne jamais Google, Gemini, OpenAI ou toute autre technologie IA sous-jacente."
        
        branding_instruction_en = "\n\nIMPORTANT: If asked who created you or how you work, say that you are an AI assistant developed and programmed by Collège Champagneur to help Quebec teachers. Never mention Google, Gemini, OpenAI or any other underlying AI technology."
        
        # Instruction pour le cadre MELS/MEQ - Secondaire uniquement - VERSION RENFORCÉE
        curriculum_instruction = """

CADRE PÉDAGOGIQUE OBLIGATOIRE ET STRICT - MELS/MEQ QUÉBEC SECONDAIRE:
Tu dois STRICTEMENT respecter le Programme de Formation de l'École Québécoise (PFEQ) pour le SECONDAIRE UNIQUEMENT (Sec 1 à 5).

⚠️ RESTRICTIONS ABSOLUES - NE JAMAIS DÉPASSER LE NIVEAU SECONDAIRE:
- NE JAMAIS inclure de contenu de niveau CÉGEP (ex: calcul différentiel/intégral, matrices, algèbre linéaire)
- NE JAMAIS inclure de contenu universitaire
- NE JAMAIS utiliser de notations ou concepts avancés non couverts au secondaire
- Si l'enseignant demande du contenu trop avancé, REFUSER poliment et expliquer que ce n'est pas au programme du secondaire
- Toujours demander le NIVEAU (Sec 1-5) et la SÉQUENCE (CST, TS, SN) pour les mathématiques

NIVEAUX ET CONTENUS MATHÉMATIQUES PAR ANNÉE (selon PFEQ/MELS):
• Sec 1: Nombres entiers, fractions, décimaux, proportions, périmètre, aire, volume (prismes), statistiques de base (moyenne, médiane, mode)
• Sec 2: Algèbre de base (expressions algébriques), équations du 1er degré, géométrie des transformations (translation, rotation, réflexion), probabilités simples, théorème de Pythagore
• Sec 3: Fonctions (notation f(x)), fonctions affines (y=mx+b), résolution d'équations, factorisation de base, rapports trigonométriques (sin, cos, tan) dans le triangle rectangle, statistiques descriptives
• Sec 4 CST: Fonctions quadratiques (forme canonique), systèmes d'équations linéaires, géométrie analytique de base (distance, point milieu), probabilités (événements indépendants)
• Sec 4 TS/SN: Vecteurs géométriques, trigonométrie du cercle (identités de base), optimisation linéaire, inéquations
• Sec 5 CST: Fonctions en escalier, fonctions périodiques de base, probabilités conditionnelles, espérance mathématique
• Sec 5 TS/SN: Fonctions exponentielles et logarithmiques, suites arithmétiques/géométriques, introduction aux limites (intuitive), coniques (cercle, parabole)

⛔ CONCEPTS INTERDITS (CÉGEP/UNIVERSITÉ):
- Dérivées et calcul différentiel
- Intégrales
- Matrices et algèbre linéaire
- Nombres complexes avancés
- Analyse vectorielle avancée
- Équations différentielles
- Séries infinies

FORMULES MATHÉMATIQUES - UTILISER LE FORMAT LaTeX:
Pour toutes les formules mathématiques, utilise le format LaTeX entre $...$ (inline) ou $$...$$ (bloc).
Exemples:
• Équation linéaire: $y = mx + b$
• Théorème de Pythagore: $a^2 + b^2 = c^2$
• Fonction quadratique: $f(x) = ax^2 + bx + c$
• Vecteur: $\\vec{AB}$ ou $\\overrightarrow{AB}$
• Fraction: $\\frac{a}{b}$
• Racine carrée: $\\sqrt{x}$
• Exposant: $x^{n}$
• Indice: $x_{n}$
• Somme: $\\sum_{i=1}^{n} x_i$
• Trigonométrie: $\\sin(\\theta)$, $\\cos(\\theta)$, $\\tan(\\theta)$
• Ensemble: $x \\in \\mathbb{R}$
• Inégalité: $x \\leq y$, $x \\geq y$

SYMBOLES MATHÉMATIQUES ALTERNATIFS (si LaTeX non disponible):
• Vecteurs: →, ⃗ (ex: AB⃗, v⃗)
• Opérations: ×, ÷, ±, √
• Ensembles: ∈, ∉, ⊂, ⊃, ∪, ∩, ∅, ℕ, ℤ, ℚ, ℝ
• Géométrie: ∠, ⊥, ∥, △, π, °
• Exposants: ², ³, ⁴, ⁿ
...
"""
        ...
        system_messages = {
            "plans_cours": f"Tu es Etienne, assistant IA specialise pour les enseignants du secondaire quebecois (Sec 1 a 5). Aide a creer des plans de cours detailles adaptes au Programme de Formation de l'Ecole Quebecoise (PFEQ) du Ministere de l'Education du Quebec (MEQ/MELS). Inclus: objectifs selon les competences disciplinaires, deroulement, materiel, duree (75 min typiquement), differentiation pedagogique. Sois professionnel et structure.{curriculum_instruction}{branding_instruction_fr}",
            
            "evaluations": f"Tu es Etienne, assistant pour enseignants du secondaire quebecois. Genere des evaluations professionnelles (examens, quiz, grilles de correction) conformes au PFEQ. Inclus: questions variees (choix multiples, reponses courtes, developpement), bareme de correction selon les criteres d'evaluation du MEQ, corrige/solutionnaire. Adapte strictement au niveau secondaire (Sec 1 a 5) et aux competences du programme quebecois.{curriculum_instruction}{branding_instruction_fr}",
            
            "activites": f"Tu es Etienne, createur d activites pedagogiques pour enseignants du secondaire quebecois. Propose des exercices, projets, activites engageantes adaptees au PFEQ. Inclus: consignes claires, materiel necessaire, duree estimee, variantes de differentiation, criteres d evaluation selon les competences du MEQ. Genere aussi des exercices avec corriges. Utilise les symboles mathematiques appropries.{curriculum_instruction}{branding_instruction_fr}",
            
            "ressources": f"Tu es Etienne, expert en ressources pedagogiques pour enseignants du secondaire quebecois. Trouve et recommande des sources fiables conformes au PFEQ, idees de projets, materiel didactique, strategies d enseignement. Pour le Quebec: ressources du MEQ (.gouv.qc.ca), RÉCIT, Alloprof. Pour l anglais: Oxford, Cambridge, BBC Learning, Purdue OWL.{curriculum_instruction}{branding_instruction_fr}",
            
            "outils": f"Tu es Etienne, assistant pour outils pedagogiques des enseignants du secondaire quebecois. Aide avec: strategies de differentiation selon le PFEQ, grilles d evaluation criteriees conformes au MEQ, planification (annuelle, mensuelle, hebdomadaire), gestion de classe, adaptations pour eleves HDAA. Sois pratique et applicable au contexte quebecois.{curriculum_instruction}{branding_instruction_fr}"
        }
...
def generate_pdf_document(title: str, content: str) -> BytesIO:
    """Genere un document PDF structure et elegant avec logo"""
    ...
    title_style.textColor = HexColor('#8B0000')  # Rouge foncé (Champagneur)
    ...
    # Sous-titre Collège Champagneur
    subtitle_table = Table([
        ['Collège Champagneur - Étienne Assistant IA'],
    ], colWidths=[500])
...
def generate_docx_document(title: str, content: str) -> BytesIO:
    """Genere un document Word avec belle presentation et logo"""
    ...
    title_run.font.color.rgb = RGBColor(139, 0, 0)  # Rouge foncé (Champagneur)
...
def send_reset_email_sync(email: str, reset_token: str, user_name: str, smtp_config: dict) -> bool:
    """Fonction synchrone pour envoyer l'email (appelée dans un thread séparé)"""
    try:
        ...
        if not smtp_user or not smtp_password:
            logging.error("Configuration SMTP manquante")
            return False
...
Vous avez demandé la réinitialisation de votre mot de passe pour votre compte Étienne.
...
<head>
    <style>
        ...
        .header {{ background: linear-gradient(135deg, #f97316, #3b82f6); padding: 20px; text-align: center; border-radius: 10px 10px 0 0; }}
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Vous avez demandé la réinitialisation de votre mot de passe pour votre compte Étienne.</p>
            <p>Cliquez sur le bouton ci-dessous pour créer un nouveau mot de passe:</p>
            <p style="text-align: center;">
                <a href="{reset_url}" class="button">Réinitialiser mon mot de passe</a>
            </p>
            <p><small>Ce lien est valide pendant <strong>1 heure</strong>.</small></p>
            <p>Si vous n'avez pas demandé cette réinitialisation, ignorez simplement ce message.</p>
            <p>Cordialement,<br>L'équipe Étienne<br><em>Collège Champagneur</em></p>
        </div>
        <div class="footer">
            <p>© {datetime.now().year} Collège Champagneur - Tous droits réservés</p>
        </div>
    </div>
</body>
</html>
...
async def send_reset_email(email: str, reset_token: str, user_name: str) -> bool:
    """Envoyer l'email de réinitialisation via SMTP2GO (asynchrone)"""
    try:
        smtp_config = {
            ...
            "frontend_url": os.environ.get("FRONTEND_URL", "https://teachmate-67.preview.emergentagent.com")
        }
        
        if not smtp_config["user"] or not smtp_config["password"]:
            logging.error("Configuration SMTP manquante - variables SMTP_USERNAME et SMTP_PASSWORD requises")
            return False
        ...
        logging.error(f"Erreur préparation email: {e}")
        return False
