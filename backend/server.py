...
AT THE END, add a "📚 Sources and References" section.{branding_instruction_en}"""
        ...
        # === GOOGLE GEMINI 2.5 FLASH - AVEC RETRY AUTOMATIQUE ===
        last_error = None
        max_attempts = 3  # 3 tentatives max (1 + 2 retries)
        
        for attempt in range(max_attempts):
            try:
                # Enregistrer la requête (seulement la première tentative)
                if attempt == 0:
                ...
                # Succès — réinitialiser le compteur de 429 consécutifs
                ...
                is_429 = "429" in error_str or "resource has been exhausted" in error_lower
                
                # Distinguer quota JOURNALIER épuisé vs rate limit temporaire
                is_hard_quota = "exceeded your current quota" in error_lower or "billing" in error_lower or "plan" in error_lower
                
                if is_429 and is_hard_quota:
                    # Quota journalier épuisé — PAS de retry, message clair
                    logging.warning(f"⛔ Quota journalier Google épuisé: {error_str[:200]}")
                    gemini_quota.record_quota_error(15)
                    return {
                        "response": "⏳ Le quota journalier de l'API Google est temporairement épuisé. "
                                    "Cela se réinitialise automatiquement (généralement dans quelques minutes à quelques heures). "
                ...
                if is_429 and attempt < max_attempts - 1:
                    # Rate limit temporaire — retry automatique
                    wait = 5 * (attempt + 1)
                    logging.info(f"🔄 Retry {attempt + 1}/{max_attempts - 1}: 429 Gemini, attente {wait}s...")
                    await asyncio.sleep(wait)
                    continue
                
                if is_429:
                    # Toutes les tentatives échouées sur rate limit temporaire
                    logging.warning(f"⚠️ 429 Gemini après {max_attempts} tentatives: {error_str[:200]}")
                    gemini_quota.record_quota_error(15)
                    return {
                        "response": "⏳ L'API d'Étienne est temporairement surchargée. Veuillez réessayer dans 15 secondes.",
                        "trust_score": None,
                        "sources": [],
                        "quota_exceeded": True,
                        "wait_seconds": 15
