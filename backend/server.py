...
# Import du générateur de graphiques (optionnel)
# Ajouter le répertoire backend au path Python pour Render
...
try:
    ...
    logging.info("✅ Module graph_generator importé avec succès - graphiques mathématiques disponibles")
except ImportError as e:
    GRAPH_GENERATOR_AVAILABLE = False
    logging.error(f"❌ Module graph_generator non disponible: {e}")
    logging.error(f"Chemin Python: {sys.path}")
    logging.error(f"Répertoire backend: {BACKEND_DIR}")
    
    # Diagnostic: Vérifier ce qui existe dans le dossier backend
    utils_dir = BACKEND_DIR / "utils"
    logging.error(f"Dossier utils existe? {utils_dir.exists()}")
    if utils_dir.exists():
        try:
            utils_contents = list(utils_dir.iterdir())
            logging.error(f"Contenu de utils/: {[f.name for f in utils_contents]}")
        except Exception as list_err:
            logging.error(f"Impossible de lister utils/: {list_err}")
    
    # Vérifier __init__.py
    init_file = utils_dir / "__init__.py"
    logging.error(f"Fichier __init__.py existe? {init_file.exists()}")
    
    try:
        import matplotlib
        import numpy
        logging.info("✅ matplotlib et numpy sont installés")
    except ImportError as lib_error:
        logging.error(f"❌ Bibliothèques manquantes: {lib_error}")
...
async def get_ai_response(message: str, message_type: str) -> dict:
    ...
    try:
        # DETECTION DE DEMANDE DE GRAPHIQUE MATHEMATIQUE (si module disponible)
        graph_result = None
        if GRAPH_GENERATOR_AVAILABLE:
            graph_result = graph_generator.process_graph_request(message)
        
        if graph_result:
            # Générer une explication avec le graphique
...
Voulez-vous que j'analyse davantage cette fonction (dérivée, racines, extremums) ?"""
            ...
            return {
                ...
                "trust_score": None,
                "sources": ["Générateur de graphiques Étienne"],
