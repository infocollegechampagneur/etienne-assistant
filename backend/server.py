    ...
    while i < len(lines):
        ...
        if line.startswith('|') and '|' in line[1:]:
            ...
            if len(table_lines) >= 2:
                header_cells = [strip_html_spans(cell.strip()).replace('**', '') for cell in table_lines[0].split('|') if cell.strip()]
                data_start = 2 if len(table_lines) > 1 and re.match(r'^[\|\-:\s]+$', table_lines[1]) else 1
                
                table_data = [header_cells]
                for table_line in table_lines[data_start:]:
                    cells = [strip_html_spans(cell.strip()).replace('**', '') for cell in table_line.split('|') if cell.strip()]
                    if cells:
        ...
        if line.startswith('|') and '|' in line[1:]:
            ...
            if len(table_lines) >= 2:
                ...
                for j, cell_text in enumerate(header_cells):
                    ...
                    cell.text = strip_html_spans(cell_text).replace('**', '')
                    for para in cell.paragraphs:
                        for run in para.runs:
                            run.bold = True
