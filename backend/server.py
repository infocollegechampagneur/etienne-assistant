    ...
    try:
        ...
        try:
            if file_extension == 'pdf':
                # Extraction PDF optimisée pour gros fichiers
                try:
                    with pdfplumber.open(temp_file_path) as pdf:
                        # Limiter à 200 pages max pour éviter timeout
                        ...
                        if len(pdf.pages) > max_pages:
                            extracted_text += f"\n\n[Note: Seules les {max_pages} premières pages ont été extraites sur {len(pdf.pages)} pages totales. Pour analyser plus de pages, divisez le document.]\n"
                
                except Exception as pdf_error:
                    logging.warning(f"pdfplumber échoué: {pdf_error}, essai PyPDF2...")
                    ...
                    try:
                        ...
                        logging.error(f"PyPDF2 aussi échoué: {pypdf_error}")
                        raise HTTPException(
                            status_code=500,
                            detail="Impossible d'extraire le texte de ce PDF. Le fichier est peut-être corrompu ou protégé."
                        )
