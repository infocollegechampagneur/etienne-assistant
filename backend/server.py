...
logging.info(f"🔑 {key_rotator.key_count()} clé(s) Google API configurée(s)")
...
AT THE END, add a "📚 Sources and References" section.{branding_instruction_en}"""
        ...
        # === GOOGLE GEMINI 2.5 FLASH — ROTATION DE CLÉS + RETRY ===
        last_error = None
        max_attempts = 3
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if is_429:
                    ...
                    key_rotator.mark_failed(api_key, cooldown=60)

                    if attempt < max_attempts - 1:
                        wait = 8 * (attempt + 1)
                        logging.info(f"🔄 Retry {attempt + 1}/{max_attempts - 1}: 429 Gemini, rotation de clé + attente {wait}s...")
                        await asyncio.sleep(wait)
                        continue

                    logging.warning(f"⚠️ 429 Gemini après {max_attempts} tentatives: {error_str[:200]}")
                    gemini_quota.record_quota_error(15)
                    return {
                        "response": "⏳ Étienne est temporairement surchargé. Veuillez patienter environ 30 secondes et réessayer.",
                        "trust_score": None,
                        "sources": [],
                        "quota_exceeded": True,
                        "wait_seconds": 15
