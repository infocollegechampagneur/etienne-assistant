        ...
        try:
            if file_extension == 'pdf':
                ...
                try:
                    ...
                    ocr_prompt = """Tu es un TRANSCRIPTEUR DIPLOMATIQUE de copies d'élèves manuscrites. Ta mission : reproduire EXACTEMENT, lettre par lettre, ce que l'élève a écrit — comme une photocopie en texte.

⛔ INTERDICTION ABSOLUE DE CORRIGER. C'est une copie d'élève destinée à être évaluée : chaque faute a de la valeur pour l'enseignant. Si tu corriges une seule faute, la transcription est INUTILISABLE.

RÈGLES STRICTES :
1) Transcris CHAQUE MOT tel qu'il est écrit, MÊME s'il est mal orthographié, mal accordé ou mal conjugué.
   ...
   - L'élève écrit « il ne » sans accent → tu écris « il ne » sans accent
2) NE complète JAMAIS un mot avec la « bonne » orthographe de ta mémoire : recopie les LETTRES visibles, une par une. Si l'élève a oublié une lettre, un accent, un « s » ou un « t » final, ta transcription doit aussi l'omettre.
3) IGNORE COMPLÈTEMENT toutes les annotations de l'ENSEIGNANT : encre ROUGE, codes (G1, U1, C, P, S, V...), cercles, soulignements, flèches, notes en marge, « Titre? ». Transcris UNIQUEMENT le texte de l'élève (encre bleue/noire ou crayon).
...
6) Si un mot est vraiment indéchiffrable, écris [illisible] — ne devine pas avec un mot bien orthographié.
7) Retourne UNIQUEMENT la transcription brute, sans commentaire.

Avant de répondre, RELIS ta transcription mot par mot en la comparant à l'image : si un mot de ta transcription est mieux orthographié que sur l'image, remplace-le par ce qui est réellement écrit."""
