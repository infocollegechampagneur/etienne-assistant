...
try:
    ...
    def generate_angle_diagram(angle_type: str) -> dict:
        """Génère un diagramme d'angles géométriques style manuel scolaire québécois"""
        try:
            ...
            # Couleurs pastel comme dans l'image (avec transparence)
            color_yellow = '#FFE566'  # Jaune pastel
            color_blue = '#99CCFF'    # Bleu pastel  
            color_green = '#99FF99'   # Vert pastel
            color_pink = '#FFB3B3'    # Rose pastel
            color_orange = '#FFD699'  # Orange pastel
            
            def draw_angle_wedge(center, radius, theta1, theta2, color, ax):
                """Dessine un secteur d'angle coloré"""
                wedge = Wedge(center, radius, theta1, theta2, facecolor=color, edgecolor='black', linewidth=1.5, alpha=0.7)
                ax.add_patch(wedge)
            
            def draw_line(p1, p2, ax, style='-', color='black', lw=1.5):
                """Dessine une ligne entre deux points"""
                ax.plot([p1[0], p2[0]], [p1[1], p2[1]], style, color=color, linewidth=lw)
            
            def draw_right_angle(center, size, rotation, ax):
                """Dessine un petit carré pour indiquer un angle droit"""
                import numpy as np
                angle_rad = np.radians(rotation)
                cos_a, sin_a = np.cos(angle_rad), np.sin(angle_rad)
                
                # Points du carré
                p1 = (center[0] + size * cos_a, center[1] + size * sin_a)
                p2 = (p1[0] + size * (-sin_a), p1[1] + size * cos_a)
                p3 = (center[0] + size * (-sin_a), center[1] + size * cos_a)
                
                ax.plot([center[0], p1[0]], [center[1], p1[1]], 'k-', lw=1)
                ax.plot([p1[0], p2[0]], [p1[1], p2[1]], 'k-', lw=1)
                ax.plot([p2[0], p3[0]], [p2[1], p3[1]], 'k-', lw=1)
            ...
            if angle_type == 'adjacents':
                ...
                draw_line(O, (4, 1.5), ax, lw=2)      # Droite x (horizontale)
                draw_line(O, (3.5, 3.5), ax, lw=2)   # Droite y
                draw_line(O, (1.5, 4), ax, lw=2)     # Droite z (verticale)
                
                # Angles colorés
                draw_angle_wedge(O, 0.6, 0, 45, color_green, ax)
                draw_angle_wedge(O, 0.6, 45, 90, color_blue, ax)
                
                # Étiquettes
                ax.text(2.3, 1.65, '38°', fontsize=14, fontweight='bold')
                ax.plot(O[0], O[1], 'ko', markersize=8)
                ax.text(O[0]-0.2, O[1]-0.4, 'O', fontsize=14, fontweight='bold')
                ax.text(4, 1.3, 'x', fontsize=14, fontweight='bold')
                ax.text(3.6, 3.6, 'y', fontsize=14, fontweight='bold')
                ax.text(1.3, 4, 'z', fontsize=14, fontweight='bold')
                
                ax.set_title('ANGLES ADJACENTS\nPartagent un sommet et un côté commun', fontsize=16, fontweight='bold', pad=20)
                ...
                # Droites
                draw_line(O, (5, 1), ax, lw=2)       # x horizontal
                draw_line(O, (2, 4), ax, lw=2)       # z vertical
                draw_line(O, (4, 2.5), ax, lw=2)     # y diagonal
                
                # Angle droit
                draw_right_angle(O, 0.25, 0, ax)
                
                # Angles colorés  
                draw_angle_wedge(O, 0.7, 0, 27, color_yellow, ax)
                draw_angle_wedge(O, 0.7, 27, 90, color_blue, ax)
                
                # Étiquettes
                ax.text(2.8, 1.2, '27°', fontsize=12, fontweight='bold')
                ax.text(2.2, 1.9, '63°', fontsize=12, fontweight='bold')
                ax.plot(O[0], O[1], 'o', color=color_yellow, markersize=12, markeredgecolor='black')
                ax.text(O[0]-0.1, O[1]+0.1, 'A', fontsize=11, fontweight='bold', ha='center', va='center')
                ax.text(5, 0.8, 'x', fontsize=14, fontweight='bold')
                ax.text(1.8, 4, 'z', fontsize=14, fontweight='bold')
                ax.text(4.1, 2.6, 'y', fontsize=14, fontweight='bold')
                
                ax.set_title('ANGLES COMPLÉMENTAIRES\n27° + 63° = 90°', fontsize=16, fontweight='bold', pad=20)
                ...
                # Droite horizontale complète (angle plat)
                draw_line((-2.5, 1.5), (3.5, 1.5), ax, lw=2)
                # Droite diagonale
                draw_line(O, (2.5, 3.5), ax, lw=2)
                
                # Angles colorés
                draw_angle_wedge(O, 0.7, 0, 53, color_green, ax)
                draw_angle_wedge(O, 0.7, 53, 180, color_pink, ax)
                
                # Étiquettes
                ax.text(1.3, 1.7, '53°', fontsize=12, fontweight='bold')
                ax.text(-0.3, 2.1, '127°', fontsize=12, fontweight='bold')
                ax.plot(O[0], O[1], 'o', color=color_orange, markersize=12, markeredgecolor='black')
                ax.text(O[0], O[1], 'O', fontsize=10, fontweight='bold', ha='center', va='center')
                ax.text(3.5, 1.3, 'x', fontsize=14, fontweight='bold')
                ax.text(-2.5, 1.3, "x'", fontsize=14, fontweight='bold')
                ax.text(2.6, 3.6, 'y', fontsize=14, fontweight='bold')
                
                ax.set_title('ANGLES SUPPLÉMENTAIRES\n53° + 127° = 180°', fontsize=16, fontweight='bold', pad=20)
                ...
                draw_line((-2.5, -1.5), (2.5, 1.5), ax, lw=2)  # Droite 1
                draw_line((-2.5, 1), (2.5, -1), ax, lw=2)      # Droite 2
                
                # Quatre angles (opposés sont de même couleur)
                draw_angle_wedge(O, 0.6, -22, 31, color_yellow, ax)   # Angle 1
                draw_angle_wedge(O, 0.6, 31, 158, color_blue, ax)     # Angle 2
                draw_angle_wedge(O, 0.6, 158, 211, color_yellow, ax)  # Angle 3 (opposé à 1)
                draw_angle_wedge(O, 0.6, 211, 338, color_blue, ax)    # Angle 4 (opposé à 2)
                
                # Étiquettes
                ax.text(0.5, 0.15, '38°', fontsize=11, fontweight='bold')
                ax.text(-0.9, -0.2, '38°', fontsize=11, fontweight='bold')
                ax.plot(O[0], O[1], 'o', color=color_orange, markersize=12, markeredgecolor='black')
                ax.text(O[0], O[1], 'O', fontsize=10, fontweight='bold', ha='center', va='center')
                
                ax.text(2.5, 1.7, 'x', fontsize=14, fontweight='bold')
                ax.text(-2.7, -1.7, "x'", fontsize=14, fontweight='bold')
                ax.text(2.5, -1.2, 'y', fontsize=14, fontweight='bold')
                ax.text(-2.7, 1.2, "y'", fontsize=14, fontweight='bold')
                
                ax.set_title('ANGLES OPPOSÉS PAR LE SOMMET\nLes angles opposés sont égaux', fontsize=16, fontweight='bold', pad=20)
                
            elif angle_type == 'alternes_internes':
                ax.set_xlim(-3, 4)
                ax.set_ylim(-2, 4)
                
                # Droites parallèles (pointillées)
                draw_line((-2.5, 2.5), (3.5, 2.5), ax, style='--', color='gray', lw=1.5)
                draw_line((-2.5, 0.5), (3.5, 0.5), ax, style='--', color='gray', lw=1.5)
                
                # Sécante (ligne solide)
                draw_line((-1, -1), (2.5, 4), ax, lw=2)
                
                # Points d'intersection
                A = (1.17, 2.5)
                B = (0.33, 0.5)
                
                # Angles alternes-internes (même couleur)
                draw_angle_wedge(A, 0.5, 180, 243, color_yellow, ax)
                draw_angle_wedge(B, 0.5, 0, 63, color_yellow, ax)
                
                # Points colorés
                ax.plot(A[0], A[1], 'o', color=color_yellow, markersize=12, markeredgecolor='black')
                ax.text(A[0], A[1], 'A', fontsize=10, fontweight='bold', ha='center', va='center')
                ax.plot(B[0], B[1], 'o', color=color_green, markersize=12, markeredgecolor='black')
                ax.text(B[0], B[1], 'B', fontsize=10, fontweight='bold', ha='center', va='center')
                
                # Étiquettes des droites
                ax.text(-2.7, 2.5, 'x', fontsize=13, fontweight='bold')
                ax.text(3.6, 2.5, "x'", fontsize=13, fontweight='bold')
                ax.text(-2.7, 0.5, 'y', fontsize=13, fontweight='bold')
                ax.text(3.6, 0.5, "y'", fontsize=13, fontweight='bold')
                ax.text(2.6, 4, 't', fontsize=13, fontweight='bold')
                
                ax.set_title('ANGLES ALTERNES-INTERNES\nSitués de part et d\'autre de la sécante, entre les parallèles', fontsize=16, fontweight='bold', pad=20)
                ax.text(0.5, -1.5, 'Les droites en pointillés sont parallèles', fontsize=11, ha='center', style='italic')
                ...
                # Angles alternes-externes (même couleur)
                draw_angle_wedge(A, 0.5, 0, 63, color_pink, ax)
                draw_angle_wedge(B, 0.5, 180, 243, color_pink, ax)
                ...
                ax.set_title('ANGLES ALTERNES-EXTERNES\nSitués de part et d\'autre de la sécante, à l\'extérieur des parallèles', fontsize=16, fontweight='bold', pad=20)
                ax.text(0.5, -1.5, 'Les droites en pointillés sont parallèles', fontsize=11, ha='center', style='italic')
                ...
                # Angles correspondants (même position, même couleur)
                draw_angle_wedge(A, 0.5, 0, 63, color_blue, ax)
                draw_angle_wedge(B, 0.5, 0, 63, color_blue, ax)
                ...
                ax.set_title('ANGLES CORRESPONDANTS\nMême position relative par rapport à la sécante', fontsize=16, fontweight='bold', pad=20)
                ax.text(0.5, -1.5, 'Les droites en pointillés sont parallèles', fontsize=11, ha='center', style='italic')
            
            else:
                ax.set_xlim(-2, 2)
                ax.set_ylim(-2, 2)
                ax.text(0, 0, f'Type d\'angle: {angle_type}', fontsize=14, ha='center')
...
Voulez-vous que j'analyse davantage cette fonction (dérivée, racines, extremums) ?"""
        ...
        if GRAPH_GENERATOR_AVAILABLE:
            ...
            if angle_type:
                # Si c'est un menu, afficher les options disponibles
                if angle_type == 'menu':
                    menu_response = """📐 **Diagrammes d'angles disponibles**

Je peux générer des diagrammes pour les types d'angles suivants. Cliquez sur l'un d'eux ou tapez le nom :

1. **Angles adjacents** - Partagent un sommet et un côté commun
2. **Angles complémentaires** - Leur somme égale 90°
3. **Angles supplémentaires** - Leur somme égale 180°
4. **Angles opposés par le sommet** - Formés par deux droites sécantes
5. **Angles alternes-internes** - Entre deux parallèles, de part et d'autre de la sécante
6. **Angles alternes-externes** - À l'extérieur des parallèles, de part et d'autre de la sécante
7. **Angles correspondants** - Même position relative par rapport à la sécante

**Exemple de demande :** "Montre-moi les angles alternes-internes" ou "angles complémentaires"
"""
                    return {
...
![Diagramme d'angles](data:image/png;base64,{angle_result['image_base64']})

---
**📚 Rappel pour le cours de mathématiques (Nouvelle grammaire PFEQ) :**

Ce concept est enseigné au **secondaire 1-2** dans le cadre de la géométrie.

**Autres types d'angles disponibles :**
- Angles adjacents
