...
try:
    ...
    def detect_graph_request_inline(text: str) -> bool:
        """Détecte si l'utilisateur demande un graphique mathématique"""
        # Mots-clés pour graphiques
        graph_keywords = [
            'trace', 'graphique', 'courbe', 'représente',
            'affiche', 'montre', 'plot', 'graph', 'chart'
        ]
        
        # Termes mathématiques
        math_terms = [
            'f(x)', 'fonction', 'équation', 'mathématique', 'math',
            'x²', 'x³', 'sin', 'cos', 'tan', 'exp', 'log',
            'polynôme', 'parabole', 'trigonométrique', 
            'problème', 'situation', 'secondaire', 'algèbre'
        ]
        
        text_lower = text.lower()
        
        # Phrases spécifiques pour graphiques mathématiques
        specific_phrases = [
        ...
        # Sinon, vérifier combinaison mot-clé + terme mathématique
        has_keyword = any(kw in text_lower for kw in graph_keywords)
...
async def generate_image_huggingface(prompt: str, negative_prompt: str = "") -> bytes:
    """Génère une image avec Hugging Face FLUX.1-schnell (GRATUIT)"""
    # Nouveau modèle: FLUX.1-schnell (rapide et gratuit)
    # Nécessite: HUGGINGFACE_API_KEY dans les variables d'environnement
