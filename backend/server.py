...
AT THE END, add a "📚 Sources and References" section.{branding_instruction_en}"""
        ...
        try:
            ...
            # Détecter erreur de quota (429) — uniquement "429" ou "quota" pour éviter les faux positifs
            if "429" in error_str or "quota" in error_str.lower() or "resource has been exhausted" in error_str.lower():
                # Extraire le délai de retry si disponible, mais PLAFONNER à 30s
                import re
                delay_match = re.search(r'retry.*?(\d+)', error_str.lower())
                if delay_match:
                    raw_delay = int(delay_match.group(1))
                    retry_delay = min(raw_delay, 30)  # MAX 30 secondes
                else:
                    retry_delay = 30
                
                logging.warning(f"⚠️ Erreur 429 Gemini: {error_str[:200]}")
