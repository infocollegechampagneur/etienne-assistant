...
export const formatMessage = (text) => {
  ...
  const renderTable = (blockLines) => {
    ...
    let html = '<div class="message-table-wrap"><table class="message-table">';
    if (header) {
      html += '<thead><tr>' + header.map(c => `<th>${applyInline(c)}</th>`).join('') + '</tr></thead>';
    }
    html += '<tbody>' + body.map(r => '<tr>' + r.map(c => `<td>${applyInline(c)}</td>`).join('') + '</tr>').join('') + '</tbody></table></div>';
    return html;
  };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('|')) {
      ...
      i--;
      formatted.push(renderTable(block));
      continue;
    }
    
    // Séparateur horizontal --- (ou ─── / ═══ imités par l'IA)
    if (/^[-─—═=_*]{3,}$/.test(trimmed)) {
      if (inList) {
        formatted.push('</ul>');
        inList = false;
      }
      formatted.push('<hr class="message-divider"/>');
      continue;
    }
    ...
    if (trimmed.match(/^[-*•]\s/) || trimmed.match(/^\d+[\.)]\s/)) {
      ...
      formatted.push(`<li>${applyInline(content)}</li>`);
      continue;
    }
    
    // Texte en gras **texte**
    let processedLine = applyInline(trimmed);
