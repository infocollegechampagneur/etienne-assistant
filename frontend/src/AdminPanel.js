...
const AdminPanel = ({ onClose }) => {
  ...
  // Éditeurs de site
  const [siteEditors, setSiteEditors] = useState({ editors: [], all_users: [] });
  ...
  useEffect(() => {
    if (activeTab === 'stats') { loadStats(); loadExpiringLicenses(); }
    if (activeTab === 'licenses') loadLicenses();
    if (activeTab === 'words') loadBlockedWords();
    if (activeTab === 'editors') loadSiteEditors();
  }, [activeTab]);

  // ==================== ÉDITEURS DE SITE ====================

  const loadSiteEditors = async () => {
    try {
      const response = await axios.get(`${API}/api/admin/site-editors`, axiosConfig);
      setSiteEditors(response.data);
    } catch (error) {
      console.warn('Erreur chargement éditeurs');
    }
  };

  const toggleSiteEditor = async (email, enabled) => {
    try {
      await axios.post(`${API}/api/admin/site-editors/toggle`, { email, enabled }, axiosConfig);
      toast.success(enabled ? `${email} est maintenant éditeur du site` : `${email} n'est plus éditeur`);
      loadSiteEditors();
    } catch (error) {
      toast.error('Erreur');
    }
  ...
  const loadExpiringLicenses = async () => {
    try {
      const response = await axios.get(`${API}/api/admin/expiring-licenses`, axiosConfig);
      setExpiringLicenses(response.data.expiring_licenses || []);
    } catch (error) {
      console.warn('Erreur chargement licences expirantes');
    }
  };
  ...
  const sendRenewalNotifications = async () => {
    ...
    try {
      const response = await axios.post(`${API}/api/admin/send-renewal-notifications`, {}, axiosConfig);
      toast.success(response.data.message);
    } catch (error) {
      toast.error('Erreur envoi des notifications');
    } finally {
  ...
  const exportUsersCSV = async () => {
    try {
      const response = await axios.get(`${API}/api/admin/export-users`, {
        ...axiosConfig,
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `etienne_utilisateurs_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const loadStats = async () => {
    try {
      ...
      setStats({ ...response.data.stats, period: response.data.period });
    } catch (error) {
  ...
  const handleOverlayMouseDown = (e) => {
    // Marquer si le mousedown a commencé sur l'overlay
  ...
  const handleOverlayClick = (e) => {
    // Fermer seulement si le clic a commencé ET terminé sur l'overlay
    if (
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                ...
                <TabsTrigger value="editors" data-testid="editors-tab">Éditeurs Site</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="stats">
                <div className="space-y-6 mt-4">
                  ...
                  <div className="flex flex-wrap gap-2 items-center justify-between">
                    <h3 className="text-xl font-semibold" data-testid="admin-stats-title">Statistiques Globales</h3>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={exportUsersCSV} data-testid="export-users-btn">
                      ...
                      {expiringLicenses.length > 0 && (
                        ...
                        >
                          {sendingNotifications ? 'Envoi...' : `Notifier renouvellements (${expiringLicenses.length})`}
                        </Button>
                      )}
                    </div>
                  </div>

                  {/* Alerte licences expirantes */}
                  {expiringLicenses.length > 0 && (
                    <Card className="border-amber-300 bg-amber-50" data-testid="expiring-licenses-alert">
                      <CardContent className="p-4">
                        <div className="flex items-center gap-2 mb-3">
                          <span className="text-lg font-semibold text-amber-800">Licences à renouveler ({expiringLicenses.length})</span>
                        </div>
                        <div className="space-y-2">
                          {expiringLicenses.map((lic, idx) => (
                            <div key={idx} className={`flex justify-between items-center p-2 rounded text-sm ${lic.is_expired ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-900'}`}>
                              <span className="font-medium">{lic.organization_name}</span>
                              <span>
                                {lic.is_expired
                                  ? `Expirée depuis ${Math.abs(lic.days_left)} jour(s)`
                                  : lic.days_left === 0 ? "Expire aujourd'hui" : `Expire dans ${lic.days_left} jour(s)`}
                              </span>
                            </div>
                          ))}
                        </div>
                  ...
                  {stats ? (
                    <>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <Card className="bg-blue-50">
                          <CardContent className="p-4">
                            ...
                            <div className="text-xs mt-1">
                              <span className="text-green-600">{stats.licenses?.active || 0} actives</span>
                              {' '}<span className="text-red-500">{stats.licenses?.expired || 0} expirées</span>
                            </div>
                          </CardContent>
                        </Card>
                        <Card className="bg-green-50">
                          <CardContent className="p-4">
                            ...
                            <div className="text-xs text-green-600 mt-1">{stats.users?.active || 0} actifs</div>
                          </CardContent>
                        </Card>
                        <Card className="bg-purple-50">
                          <CardContent className="p-4">
                            ...
                            <div className="text-xs text-purple-600 mt-1">Ce mois: {stats.logins?.this_month || 0}</div>
                          </CardContent>
                        </Card>
                        <Card className="bg-orange-50">
                          <CardContent className="p-4">
                            ...
                            <div className="text-xs text-orange-600 mt-1">Ce mois: {stats.requests?.this_month || 0}</div>
                          </CardContent>
                        </Card>
                      </div>

                      {/* Répartition licences (donut visuel) + Requêtes par période */}
                      <div className="grid md:grid-cols-2 gap-6">
                        <Card>
                          <CardHeader><CardTitle className="text-lg">Répartition des Licences</CardTitle></CardHeader>
                          <CardContent>
                            {(() => {
                              const active = stats.licenses?.active || 0;
                              const expired = stats.licenses?.expired || 0;
                              const inactive = Math.max((stats.licenses?.total || 0) - active - expired, 0);
                              const total = active + expired + inactive || 1;
                              const segments = [
                                { label: 'Actives', value: active, color: '#22c55e' },
                                { label: 'Expirées', value: expired, color: '#ef4444' },
                                { label: 'Inactives', value: inactive, color: '#9ca3af' },
                              ].filter(s => s.value > 0);
                              let cumPercent = 0;
                              return (
                                <div className="flex items-center gap-6">
                                  <svg viewBox="0 0 36 36" className="w-28 h-28 flex-shrink-0">
                                    {segments.map((seg, i) => {
                                      ...
                                      cumPercent += percent;
                                      return (
                                        <circle key={i} cx="18" cy="18" r="15.9155" fill="none" stroke={seg.color} strokeWidth="3.5"
                                          strokeDasharray={dashArray} strokeDashoffset={offset} />
                                      );
                                    })}
                                    <text x="18" y="18.5" textAnchor="middle" fontSize="7" fontWeight="bold" fill="#374151">{stats.licenses?.total || 0}</text>
                                    <text x="18" y="23" textAnchor="middle" fontSize="3" fill="#9ca3af">total</text>
                                  </svg>
                                  <div className="space-y-2 text-sm">
                                    {segments.map((seg, i) => (
                                      <div key={i} className="flex items-center gap-2">
                                        <span className="w-3 h-3 rounded-full inline-block" style={{ background: seg.color }}></span>
                        ...
                        <Card>
                          <CardHeader><CardTitle className="text-lg">Requêtes par Période</CardTitle></CardHeader>
                          <CardContent>
                            <div className="space-y-3">
                              <div className="flex justify-between items-center p-2 bg-blue-50 rounded">
                                <span>Ce mois ({stats.period?.current_month}/{stats.period?.current_year})</span>
                                <span className="font-bold text-blue-600">{stats.requests?.this_month || 0}</span>
                              </div>
                              <div className="flex justify-between items-center p-2 bg-green-50 rounded">
                                <span>Cette année ({stats.period?.current_year})</span>
                                <span className="font-bold text-green-600">{stats.requests?.this_year || 0}</span>
                              </div>
                              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                                <span>Total historique</span>
                                <span className="font-bold text-gray-700">{stats.requests?.total || 0}</span>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
                      ...
                      {/* Graphique Requêtes 12 derniers mois (amélioré) */}
                      {stats.requests?.by_month && (
                        <Card>
                          <CardHeader><CardTitle className="text-lg">Requêtes — 12 derniers mois</CardTitle></CardHeader>
                          <CardContent>
                            <div className="flex items-end gap-1" style={{ height: 140 }}>
                              {stats.requests.by_month.slice().reverse().map((m, idx) => {
                                ...
                                return (
                                  <div key={idx} className="flex-1 flex flex-col items-center gap-1" title={`${monthNames[m.month]} ${m.year}: ${m.count} requêtes`}>
                                    <span className="text-[10px] font-medium text-gray-600">{m.count || ''}</span>
                                    <div
                                      className="w-full rounded-t transition-all"
                                      style={{ height: `${Math.max(height, 3)}%`, background: idx === stats.requests.by_month.length - 1 ? '#f97316' : '#fdba74' }}
                                    ></div>
                                    <span className="text-[9px] text-gray-500">{monthNames[m.month]}</span>
                                  </div>
                                );
                              })}
                            </div>
                      ...
                      {/* Top Utilisateurs + Tableau détaillé */}
                      <div className="grid md:grid-cols-2 gap-6">
                        <Card>
                          <CardHeader><CardTitle className="text-lg">Top Utilisateurs ce Mois</CardTitle></CardHeader>
                          <CardContent>
                            {stats.top_users?.this_month?.length > 0 ? (
                              <div className="space-y-2">
                                {stats.top_users.this_month.map((user, idx) => {
                                  const max = stats.top_users.this_month[0]?.count || 1;
                                  return (
                                    <div key={idx} className="space-y-1">
                                      ...
                                      <div className="w-full bg-gray-100 rounded-full h-1.5">
                                        <div className="h-1.5 rounded-full bg-orange-400" style={{ width: `${(user.count / max) * 100}%` }}></div>
                                      </div>
                                    </div>
                                  );
                                })}
                        ...
                        <Card>
                          <CardHeader><CardTitle className="text-lg">Top Utilisateurs cette Année</CardTitle></CardHeader>
                          <CardContent>
                            {stats.top_users?.this_year?.length > 0 ? (
                              <div className="space-y-2">
                                {stats.top_users.this_year.map((user, idx) => {
                                  const max = stats.top_users.this_year[0]?.count || 1;
                                  return (
                                    <div key={idx} className="space-y-1">
                                      <div className="flex justify-between text-sm">
                                        <span>{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}.`} {user.email}</span>
                                      ...
                                      <div className="w-full bg-gray-100 rounded-full h-1.5">
                                        <div className="h-1.5 rounded-full bg-blue-400" style={{ width: `${(user.count / max) * 100}%` }}></div>
                                      </div>
                                    </div>
                                  );
                                })}
                              ...
                              <p className="text-gray-500 text-center">Aucune donnée cette année</p>
                            )}
                          </CardContent>
                        </Card>
                      </div>
                      ...
                      <Card>
                        <CardHeader><CardTitle className="text-lg">Statistiques Détaillées par Utilisateur</CardTitle></CardHeader>
                        <CardContent>
              ...
              {/* ===== ONGLET ÉDITEURS DE SITE ===== */}
              <TabsContent value="editors">
                <div className="space-y-4 mt-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold" data-testid="editors-title">Éditeurs du site</h3>
                    <p className="text-sm text-gray-500">Les éditeurs peuvent modifier le design via le bouton "Design"</p>
                  </div>

                  {/* Éditeurs actuels */}
                  {siteEditors.editors?.length > 0 && (
                    <Card className="border-green-200 bg-green-50">
                      <CardContent className="p-4">
                        <h4 className="font-medium text-green-800 mb-3">Éditeurs actifs ({siteEditors.editors.length})</h4>
                        <div className="space-y-2">
                          {siteEditors.editors.map((editor, i) => (
                            <div key={i} className="flex items-center justify-between bg-white p-3 rounded-lg border">
                              <div>
                                <span className="font-medium text-sm">{editor.full_name}</span>
                                <span className="text-gray-500 text-xs ml-2">{editor.email}</span>
                              </div>
                              <Button size="sm" variant="outline" className="border-red-200 text-red-600 hover:bg-red-50"
                                onClick={() => toggleSiteEditor(editor.email, false)} data-testid={`remove-editor-${i}`}>
                                Retirer
                              </Button>
                            </div>
                          ))}
                  ...
                  {/* Tous les utilisateurs — assigner */}
                  <Card>
                    <CardContent className="p-4">
                      <h4 className="font-medium text-gray-800 mb-3">Assigner un éditeur</h4>
                      <div className="space-y-2 max-h-80 overflow-y-auto">
                        {siteEditors.all_users?.filter(u => !u.site_editor).map((user, i) => (
                          <div key={i} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-200">
                            <div>
                            ...
                            <Button size="sm" variant="outline" className="border-green-200 text-green-600 hover:bg-green-50"
                              onClick={() => toggleSiteEditor(user.email, true)} data-testid={`add-editor-${i}`}>
                              Assigner
                            </Button>
                          </div>
                        ))}
                        {siteEditors.all_users?.filter(u => !u.site_editor).length === 0 && (
                          <p className="text-center text-gray-500 py-4">Tous les utilisateurs sont déjà éditeurs ou aucun utilisateur disponible</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
