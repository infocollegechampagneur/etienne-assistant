...
const LicenseAdminPanel = ({ onClose }) => {
  ...
  // Ref pour l'overlay - DOIT être déclaré avant tout return conditionnel
  ...
  const loadLicenseData = async () => {
    try {
      ...
      if (err.response?.status === 403) {
        setError("Vous n'êtes pas administrateur d'une licence. Ce panneau est réservé aux administrateurs de licence.");
      } else {
        setError('Erreur lors du chargement des données de licence.');
      }
    }
  };
  ...
  const addUser = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Erreur ajout utilisateur');
    } finally {
      setLoading(false);
    }
  };
  ...
  const removeUser = async (userId, userEmail) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Erreur suppression utilisateur');
    }
  };
  ...
  const updateLicenseKey = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Erreur modification clé');
    } finally {
      setLoading(false);
    }
  };
  ...
  const changeUserEmail = async (oldEmail) => {
    ...
    try {
      const response = await axios.post(
        ...
        { old_email: oldEmail, new_email: newUserEmail },
        axiosConfig
      );
      ...
      toast.error(err.response?.data?.detail || 'Erreur lors du changement d\'email');
    } finally {
        ...
        <Card className="w-full max-w-md" onClick={(e) => e.stopPropagation()}>
          <CardContent className="p-8 text-center">
            <div className="text-gray-500">
              ...
              <p>Chargement des données de licence...</p>
              <button 
                onClick={onClose} 
                  ...
                  <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <Card className="bg-green-50">
                        <CardContent className="p-4 text-center">
                          <div className="text-2xl font-bold text-green-600">{stats.requests?.total || 0}</div>
            ...
            {activeView === 'users' && (
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="font-semibold">👥 Utilisateurs ({users.length}/{license.max_users})</h3>
                  {users.length === 0 ? (
                ...
                <Card className="border-orange-200">
                  ...
                  <CardContent>
                    ...
                    {showKeyChange ? (
                      ...
                      <Button onClick={() => setShowKeyChange(true)} variant="outline" className="w-full">
