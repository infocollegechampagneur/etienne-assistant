...
export const HeaderNav = ({
  ...
  const headerConf = siteConfig?.header || {};
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 flex-wrap">
            ...
            {currentUser ? (
              <>
                {headerConf.show_user_badge !== false && (
                <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
                  {currentUser.full_name}
                </Badge>
                )}
                {isLicenseAdmin && !isAdmin && (
                  <Button size="sm" onClick={() => setShowLicenseAdminPanel(true)} className="bg-blue-600 hover:bg-blue-700">Ma licence</Button>
                )}
                {isAdmin && (
                  <>
                    <Button size="sm" onClick={() => setShowAdminPanel(true)} className="bg-orange-600 hover:bg-orange-700">Admin</Button>
                  </>
                )}
                {(isAdmin || isSiteEditor) && (
                    <Button size="sm" variant="outline" onClick={() => setShowSiteConfig(true)} className="border-purple-200 hover:bg-purple-50 text-purple-700" data-testid="site-config-btn">Design</Button>
                )}
                {modules.nouveautes_button !== false && (
                <Button size="sm" variant="outline" onClick={() => setShowNouveautes(true)} className="border-orange-200 hover:bg-orange-50" data-testid="nouveautes-btn">
                ...
                {headerConf.show_profile_button !== false && (
                <Button size="sm" variant="outline" onClick={() => setShowProfileModal(true)} className="border-blue-200 hover:bg-blue-50">Profil</Button>
                )}
                {headerConf.show_logout_button !== false && (
                <Button size="sm" variant="outline" onClick={handleLogout}>Déconnexion</Button>
                )}
              </>
            ) : (
              <Button size="sm" onClick={() => setShowAuthModal(true)}>Connexion</Button>
            )}
          </div>
        </div>
      </div>
