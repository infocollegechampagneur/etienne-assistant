...
function App() {
  ...
  const sendMessage = async (e) => {
    ...
    // Créer une conversation cloud si c'est le premier message
    ...
    // Sauvegarder le message utilisateur dans le cloud
    if (convId && currentUser) {
    ...
    try {
      ...
      const response = await axios.post(`${API}/chat`, {
        ...
        conversation_history: updatedHistory.slice(-10) // Envoyer les 10 derniers messages pour le contexte
      });
      ...
      // Sauvegarder la réponse dans le cloud
      if (convId && currentUser) {
        await ConversationService.addMessage(convId, {
          role: 'assistant',
