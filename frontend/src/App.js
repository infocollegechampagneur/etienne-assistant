...
function App() {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (files.length > maxFiles) {
      toast.error(`Maximum ${maxFiles} fichiers à la fois`);
      return;
    }
    ...
    try {
      // Upload de tous les fichiers en parallèle pour plus de rapidité
      const uploadPromises = files.map(async (file, index) => {
        ...
        const response = await axios.post(`${API}/api/upload-file`, formData, {
          ...
          // Timeout augmenté pour PDF jusqu'à 200 pages
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        errorMessage = 'Le fichier est trop volumineux ou complexe. Timeout dépassé (120s). Essayez un fichier plus petit ou moins de pages.';
  ...
  const sendMessageWithFile = async (e) => {
    ...
    if (uploadedFile) {
      ...
      try {
        ...
        const response = await axios.post(`${API}/api/analyze-file`, {
          question: messageToSend,
                    ...
                    {isUploading && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        ...
                        <div className="text-xs text-blue-600 mt-1">
                          Timeout: 120 secondes max (PDF jusqu'à 200 pages)
                        </div>
                      </div>
                    )}
