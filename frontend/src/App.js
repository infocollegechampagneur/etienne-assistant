...
function App() {
  ...
  const messageTypes = {
    ...
    verification: {
      ...
      description: 'Détectez l\'IA et vérifiez l\'originalité',
      placeholder: 'Collez votre texte ici pour vérifier s\'il a été généré par IA ou s\'il y a du plagiat...',
      icon: '🔍'
    }
  };
  ...
  const sendMessage = async (e) => {
    ...
    try {
      // Détecter si c'est une demande de génération d'image
      const imageKeywords = ['génère une image', 'crée une image', 'génère-moi une image', 'fais-moi une image', 'dessine', 'illustre'];
      const isImageRequest = imageKeywords.some(keyword => messageToSend.toLowerCase().includes(keyword));
      ...
      if (isImageRequest) {
        ...
        if (imageResponse.data.success) {
          const imageMessage = {
            id: Date.now() + 1,
            message: "Voici l'image générée :",
            isUser: false,
            type: activeTab,
            image_base64: imageResponse.data.image_base64,
            timestamp: new Date()
          };
          setMessages(prev => [...prev, imageMessage]);
          toast.success('Image générée avec succès!');
        } else {
          throw new Error(imageResponse.data.error || 'Erreur génération image');
        }
      } else {
        // Message normal
      ...
      <section className="relative py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="text-4xl font-bold text-gray-900 mb-4">
            Étienne, votre assistant IA pour <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-blue-600">réussir vos études</span>
          </h2>
