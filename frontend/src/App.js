...
function App() {
  ...
  const [isSiteEditor, setIsSiteEditor] = useState(false);
  const [showMathSymbols, setShowMathSymbols] = useState(false);
  const [isChatFullscreen, setIsChatFullscreen] = useState(false);
  const [showProfileModal, setShowProfileModal] = useState(false);
  const [showCorrectionModal, setShowCorrectionModal] = useState(false);
  const [batchProgress, setBatchProgress] = useState(null);
  const [lastBatchResults, setLastBatchResults] = useState(null);
  ...
  const messageTypes = {
    plans_cours: {
      title: siteConfig?.layout?.chat_tabs?.plans_cours?.title || 'Plans de cours',
      description: siteConfig?.layout?.chat_tabs?.plans_cours?.description || 'Créez des planifications détaillées adaptées au programme (Sec 1-5)',
      placeholder: siteConfig?.layout?.chat_tabs?.plans_cours?.placeholder || 'Ex: Crée un plan de cours sur la photosynthèse pour Secondaire 4...',
      icon: siteConfig?.layout?.chat_tabs?.plans_cours?.icon || '📚',
      visible: siteConfig?.layout?.chat_tabs?.plans_cours?.visible !== false,
      order: siteConfig?.layout?.chat_tabs?.plans_cours?.order ?? 0,
    },
    evaluations: {
      title: siteConfig?.layout?.chat_tabs?.evaluations?.title || 'Évaluations',
      description: siteConfig?.layout?.chat_tabs?.evaluations?.description || 'Générez examens, quiz, grilles de correction professionnelles',
      placeholder: siteConfig?.layout?.chat_tabs?.evaluations?.placeholder || 'Ex: Génère un examen de français Sec 3 sur le roman avec corrigé...',
      icon: siteConfig?.layout?.chat_tabs?.evaluations?.icon || '📝',
      visible: siteConfig?.layout?.chat_tabs?.evaluations?.visible !== false,
      order: siteConfig?.layout?.chat_tabs?.evaluations?.order ?? 1,
    },
    activites: {
      title: siteConfig?.layout?.chat_tabs?.activites?.title || 'Activités',
      description: siteConfig?.layout?.chat_tabs?.activites?.description || 'Créez exercices, projets, activités pédagogiques engageantes',
      placeholder: siteConfig?.layout?.chat_tabs?.activites?.placeholder || 'Ex: Propose 3 activités interactives sur les fractions pour Sec 1...',
      icon: siteConfig?.layout?.chat_tabs?.activites?.icon || '🎯',
      visible: siteConfig?.layout?.chat_tabs?.activites?.visible !== false,
      order: siteConfig?.layout?.chat_tabs?.activites?.order ?? 2,
    },
    ressources: {
      title: siteConfig?.layout?.chat_tabs?.ressources?.title || 'Ressources',
      description: siteConfig?.layout?.chat_tabs?.ressources?.description || 'Trouvez idées, matériel pédagogique, sources fiables',
      placeholder: siteConfig?.layout?.chat_tabs?.ressources?.placeholder || 'Ex: Trouve-moi des idées de projets en histoire Sec 4...',
      icon: siteConfig?.layout?.chat_tabs?.ressources?.icon || '🔍',
      visible: siteConfig?.layout?.chat_tabs?.ressources?.visible !== false,
      order: siteConfig?.layout?.chat_tabs?.ressources?.order ?? 3,
    },
    outils: {
      title: siteConfig?.layout?.chat_tabs?.outils?.title || 'Outils',
      description: siteConfig?.layout?.chat_tabs?.outils?.description || 'Plans d\'intervention, rapports, formulaires, procédures pour tout le personnel',
      placeholder: siteConfig?.layout?.chat_tabs?.outils?.placeholder || 'Ex: Crée une grille d\'évaluation pour un exposé oral, un plan d\'intervention TDAH, une lettre aux parents...',
      icon: siteConfig?.layout?.chat_tabs?.outils?.icon || '⚙️',
      visible: siteConfig?.layout?.chat_tabs?.outils?.visible !== false,
      order: siteConfig?.layout?.chat_tabs?.outils?.order ?? 4,
    }
  };

  // Onglets triés et filtrés selon la config
  const visibleTabs = Object.entries(messageTypes)
    .filter(([, v]) => v.visible)
  ...
  useEffect(() => {
    ...
    const loadSiteConfig = async () => {
      try {
        ...
        console.warn('Config site non disponible, utilisation des défauts');
      }
    };
    loadSiteConfig();
    ...
    if (storedUser && storedToken) {
      ...
      setIsSiteEditor(user.site_editor === true);
  ...
  // Appliquer les couleurs et CSS personnalisés du site
  useEffect(() => {
    if (!siteConfig?.colors) return;
    const c = siteConfig.colors;
    const root = document.documentElement;
    root.style.setProperty('--color-primary', c.primary || '#f97316');
    root.style.setProperty('--color-secondary', c.secondary || '#2563eb');
    root.style.setProperty('--color-accent', c.accent || '#ea580c');
    root.style.setProperty('--color-bg-from', c.bg_from || '#fef3e2');
    root.style.setProperty('--color-bg-via', c.bg_via || '#ffffff');
    root.style.setProperty('--color-bg-to', c.bg_to || '#eff6ff');
    root.style.setProperty('--color-text-primary', c.text_primary || '#1f2937');

    // Injecter le CSS personnalisé
    let styleEl = document.getElementById('site-custom-css');
    if (!styleEl) {
      styleEl = document.createElement('style');
      styleEl.id = 'site-custom-css';
      document.head.appendChild(styleEl);
    }
    styleEl.textContent = siteConfig.custom_css || '';

    // Appliquer l'image de fond si configurée
    if (siteConfig.images?.background_image) {
      document.body.style.backgroundImage = `url(${siteConfig.images.background_image})`;
  ...
  // Raccourcis pour les textes et images configurés
  const t = siteConfig?.texts || {};
  const img = siteConfig?.images || {};
  ...
  const handleCorrectionSubmit = async (formattedMessage, existingConvId = null) => {
    setActiveTab('evaluations');
    setShowCorrectionModal(false);

    // Directly trigger the send flow with the formatted message
    if (!formattedMessage.trim()) return existingConvId;
    ...
    let correctionResponseText = null;
    let convId = existingConvId || currentConversationId;
    if (currentUser) {
      try {
        if (!convId) {
    ...
    try {
      ...
      correctionResponseText = response.data.response;
      ...
      // Note: la réponse IA est désormais persistée côté serveur (POST /api/chat)
    } catch (error) {
    ...
    return { convId, responseText: correctionResponseText };
  };

  const handleBatchCorrectionSubmit = async (messagesList) => {
    setActiveTab('evaluations');
    setShowCorrectionModal(false);
    ...
    for (let i = 0; i < messagesList.length; i++) {
      setBatchProgress({ current: i + 1, total: messagesList.length, name: messagesList[i].name || '' });
      const res = await handleCorrectionSubmit(messagesList[i].message, convId);
      if (res?.convId) convId = res.convId;
      if (res?.responseText) {
        results.push({ name: messagesList[i].name || `Élève ${i + 1}`, content: res.responseText });
      }
  ...
  const downloadBatchWord = async () => {
    if (!lastBatchResults?.length) return;
    try {
      const res = await axios.post(`${API}/export-batch-word`, {
        title: 'Corrections en lot',
        corrections: lastBatchResults
      }, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a');
      a.href = url;
      a.download = `Corrections_lot_${new Date().toISOString().slice(0, 10)}.docx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
    } catch (err) {
      console.error('Erreur export lot Word:', err);
    }
  };
...
**Recommandation:** ${plagiarism_result.recommendation}`;
  ...
  return (
    <div className="min-h-screen" style={{ background: `linear-gradient(135deg, ${colors.bg_from || '#fef3e2'}, ${colors.bg_via || '#ffffff'}, ${colors.bg_to || '#eff6ff'})` }}>
      ...
      <TextCorrectionModal
        ...
        onSubmitBatch={handleBatchCorrectionSubmit}
        apiUrl={API}
      />
      ...
      {/* Modal Config Site (Super Admin) */}
      <SiteConfigAdmin
        open={showSiteConfig}
        onClose={() => { setShowSiteConfig(false); /* Recharger config */ axios.get(`${API}/site-config`).then(r => setSiteConfig(r.data)).catch(() => {}); }}
      />
      ...
      <HeaderNav
        ...
        setShowSiteConfig={setShowSiteConfig}
        handleLogout={handleLogout}
        siteConfig={siteConfig}
      />

      {/* Affichage du quota en position fixe */}
      {modules.quota_widget !== false && (
      ...
      {/* Hero Section */}
      {modules.hero_section !== false && (
      <section className="relative px-4" style={{ paddingTop: `${sizes.hero_padding || 48}px`, paddingBottom: `${sizes.hero_padding || 48}px` }}>
        <div className="max-w-4xl mx-auto text-center">
          {(sizes.hero_image_size || 128) > 0 && (
          <div className="mb-8">
            <img 
              src={img.hero_image || "https://images.unsplash.com/photo-1614492898637-435e0f87cef8?crop=entropy&cs=srgb&fm=jpg&ixid=M3w3NDk1ODF8MHwxfHNlYXJjaHwyfHwlQzMlQTl0dWRpYW50JTIwbHljJUMzJUE5ZXxlbnwwfHx8fDE3NTk0MTA1OTF8MA&ixlib=rb-4.1.0&q=85"} 
              alt="Hero" 
              className="rounded-full mx-auto object-cover shadow-lg"
              style={{ width: `${sizes.hero_image_size || 128}px`, height: `${sizes.hero_image_size || 128}px` }}
            />
          </div>
          )}
          <h2 className="text-4xl font-bold mb-4" style={{ color: colors.text_primary || '#1f2937' }}>
            {t.hero_title_before || 'Étienne, votre assistant IA pour'}{' '}
            <span className="text-transparent bg-clip-text" style={{ backgroundImage: `linear-gradient(to right, ${colors.primary || '#f97316'}, ${colors.secondary || '#2563eb'})`, WebkitBackgroundClip: 'text' }}>
              {t.hero_highlight || 'le personnel scolaire'}
            </span>
          </h2>
          <p className="text-xl text-gray-600 mb-8 max-w-3xl mx-auto leading-relaxed">
            {t.hero_description || 'Créez des plans de cours, générez des documents professionnels, trouvez des ressources adaptées au programme québécois.'}
          </p>
        </div>
      </section>
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 pb-8">
        ...
        {!currentUser ? (
          <Card className="max-w-2xl mx-auto bg-white/90 backdrop-blur-sm border-orange-200">
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">
                {t.login_title || 'Connexion Requise'}
              </h3>
              <p className="text-gray-600 mb-6">
                {t.login_text || 'Vous devez vous connecter avec une licence valide pour accéder à Étienne.'}
              </p>
              <Button 
                size="lg"
                onClick={() => setShowAuthModal(true)}
                style={{ background: `linear-gradient(to right, ${colors.primary || '#f97316'}, ${colors.secondary || '#2563eb'})` }}
              >
                🔐 Se connecter
              </Button>
            </CardContent>
          ...
          <div className={`grid gap-6 ${modules.subjects_sidebar !== false ? 'lg:grid-cols-4' : 'lg:grid-cols-1 max-w-4xl mx-auto'}`}>
            
            {/* Sidebar - Matières */}
            {modules.subjects_sidebar !== false && (
          ...
          {/* Chat Interface */}
          <div className={isChatFullscreen ? "fixed inset-0 z-50 bg-white p-4 overflow-auto" : `${modules.subjects_sidebar !== false ? 'lg:col-span-3' : ''} relative`}>
            ...
            <Card className={`bg-white/90 backdrop-blur-sm border-orange-100 flex flex-col ${isChatFullscreen ? 'h-full' : 'min-h-[600px]'}`}>
              <CardHeader className="pb-2 relative">
                ...
                {!isChatFullscreen && (
                  <div className="absolute top-2 right-2 z-10 flex gap-2">
                    ...
                    {modules.correction_button !== false && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setShowCorrectionModal(true)}
                    ...
                    {/* Bouton plein écran */}
                    {modules.fullscreen_button !== false && (
                    <Button
                      variant="outline"
                      size="sm"
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <TabsList className={`grid w-full bg-gray-50`} style={{ gridTemplateColumns: `repeat(${visibleTabs.length}, minmax(0, 1fr))` }}>
                    {visibleTabs.map(([key, type]) => (
              ...
              <CardContent className="flex-1 flex flex-col">
                ...
                <div 
                  ...
                  style={{ 
                    maxHeight: isChatFullscreen ? 'calc(100vh - 350px)' : `${sizes.chat_height || 550}px`,
                  }}
                >
                  <div className="space-y-4 p-4">
                    {messages.length === 0 ? (
                      <div className="text-center py-8">
                        <div className="mb-4">
                          <img 
                            src={img.chat_empty_image || "https://images.unsplash.com/photo-1757143137415-0790a01bfa6d?crop=entropy&cs=srgb&fm=jpg&ixid=M3w3NDk1ODF8MHwxfHNlYXJjaHwzfHwlQzMlQTl0dWRpYW50JTIwbHljJUMzJUE5ZXxlbnwwfHx8fDE3NTk0MTA1OTF8MA&ixlib=rb-4.1.0&q=85"} 
                            alt="Chat vide" 
                            className="w-20 h-20 rounded-full mx-auto object-cover"
                          />
                        </div>
                        <p className="text-gray-500 mb-4">{t.chat_empty || 'Commencez à créer du matériel pédagogique'}</p>
                        <p className="text-sm text-gray-400">{t.chat_empty_sub || "Choisissez un type d'outil ci-dessus pour générer plans de cours, évaluations, activités..."}</p>
                      </div>
                    ) : (
                ...
                {activeTab === 'verification' ? (
                  ...
                  <>
                  {batchProgress && (
                    <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-lg px-3 py-2 mb-2" data-testid="batch-progress-banner">
                      <div className="w-3 h-3 border-2 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
                      <p className="text-xs text-orange-700 font-medium">
                        Correction en lot : texte {batchProgress.current}/{batchProgress.total}{batchProgress.name ? ` — ${batchProgress.name}` : ''} en cours... (une pause est faite entre chaque texte pour respecter les limites)
                      </p>
                    </div>
                  )}
                  {lastBatchResults && !batchProgress && (
                    <div className="flex items-center justify-between gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2 mb-2" data-testid="batch-done-banner">
                      <p className="text-xs text-green-700 font-medium">
                        📚 Lot terminé — {lastBatchResults.length} correction{lastBatchResults.length > 1 ? 's' : ''} ({lastBatchResults.map(r => r.name).join(', ')})
                      </p>
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          type="button"
                          onClick={downloadBatchWord}
                          className="text-xs bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-lg font-medium transition-colors"
                          data-testid="download-batch-word-btn"
                        >
                          📄 Télécharger le Word (1 page par élève)
                        </button>
                        <button type="button" onClick={() => setLastBatchResults(null)} className="text-green-400 hover:text-green-700" data-testid="dismiss-batch-banner">✕</button>
                      </div>
                    </div>
                  )}
                  <form onSubmit={sendMessageWithFile} className="space-y-2">
                    ...
                    {modules.file_upload_info !== false && (
                    <div className="text-xs text-gray-500 flex items-center gap-2">
                      <span>{t.file_formats || '📎 Formats: PDF, Word, Excel, PowerPoint, TXT, CSV'}</span>
                      <span>•</span>
          ...
          {modules.features_section !== false && (
          <FeatureSection 
      ...
      {siteConfig?.footer?.enabled && (
        <footer className="mt-8 py-6 text-center" style={{ backgroundColor: siteConfig.footer.bg_color || '#1f2937', color: siteConfig.footer.text_color || '#9ca3af' }} data-testid="site-footer">
          <p className="text-sm">{siteConfig.footer.text}</p>
          {siteConfig.footer.links?.length > 0 && (
            <div className="flex justify-center gap-6 mt-3">
              {siteConfig.footer.links.map((link, i) => (
                <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="text-sm underline hover:opacity-80" style={{ color: siteConfig.footer.text_color || '#9ca3af' }}>
                  {link.label}
                </a>
              ))}
            </div>
